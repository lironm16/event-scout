...
const { classifyAccessFromText, ACCESS_RULES } = require("./access");

function mapAccess(lobbyEntry, detailJson, opts = {}) {
  // Strongest signal: city category. This is a single hard label
  // chosen by the city CMS editor.
  const fromCategory = classifyAccessFromText(lobbyEntry?.category?.name);
  if (fromCategory) return fromCategory;

  // Fallback: cluster names. The city sometimes leaves category
  // generic ("שבועות 2026") but tags the event into a community
  // cluster ("הקומה השנייה" — young adults venue — doesn't
  // trigger, but "המרכז הגאה" would).
  for (const c of lobbyEntry?.cluster || []) {
    const hit = classifyAccessFromText(c?.name);
    if (hit) return hit;
  }
  for (const c of detailJson?.content?.cluster || []) {
    const hit = classifyAccessFromText(c?.name);
    if (hit) return hit;
  }

  // Last resort: venue name from the structured location. Catches
  // the case where neither category nor cluster mention the
  // community but the venue is identifying (e.g. an LGBTQ event
  // at "המרכז הגאה" tagged only as "שבועות 2026").
  //
  // For umbrella children the CALLER passes `opts.venueName`
  // explicitly — the child's own physical venue. This avoids
  // inheriting the parent's misleading venue (CMS errors like
  // shavuot-2026 listing its `eventLocation.name` as "המרכז
  // הגאה" while every actual child runs at a different community
  // centre across the city). When `opts.venueName` is null we
  // skip the venue heuristic entirely — that's the explicit
  // signal "no reliable venue to read from for THIS row".
  let venueName;
  ...
  if (venueName) {
    const fromVenue = classifyAccessFromText(venueName);
    if (fromVenue) return fromVenue;
  }

  // Umbrella editorial intent — last fallback before defaulting to
  // 'open'. The city CMS encodes audience intent in the umbrella's
  // slug (via detailsLink.url → "lectures-for-age-60-and-over",
  // "shavuot-2026") and title ("מגוון הרצאות מרתקות לאזרחים
  // ותיקים ברחבי העיר"). These are reliable signals for
  // senior-targeted programming (and any future community where the
  // CMS spells out the audience in the umbrella metadata) but only
  // when no STRONGER local signal (category / cluster / venue on
  // the child or single event) has fired. That keeps the "child
  // wins" policy intact: a senior umbrella whose child happens to
  // run at "המרכז הגאה" still classifies the child as
  // community-lgbtq.
  //
  // Same code path serves the umbrella's own row (lobbyEntry is
  // the umbrella record) AND every child (lobbyEntry stays the
  // umbrella — buildCityChildEventRow passes it through unchanged),
  // so one rule reconciles both.
  const umbrellaSlug = extractSlug(lobbyEntry?.detailsLink?.url) || null;
  const umbrellaText =
    [umbrellaSlug, lobbyEntry?.title].filter(Boolean).join(" ").trim();
  if (umbrellaText) {
    const fromUmbrella = classifyAccessFromText(umbrellaText);
...
function extractTagNames(lobbyEntry, detailJson) {
  ...
  for (const t of extractAudienceSubtypeTags(lobbyEntry?.audienceType)) {
    out.add(t);
  }
...
function extractAudienceSubtypeTags(audienceTypeArray) {
...
// Extract the body description from a detail JSON response. The city
// CMS stores it as a small HTML blob under `content.description` —
// usually a few `<p>` paragraphs, sometimes with inline `<a>` / `<strong>` /
// `<br>` markup. We flatten to plain text because:
//   1. Gemini reads it better without markup noise.
//   2. The events.description column (sql/041) is TEXT, not rich HTML,
//      and we surface it to the user as-is via the bot.
//
// The shape mirrors the Smarticket extractDescription path in
// lib/eventEnricher.js — both produce a single whitespace-normalized
// string that Gemini can ingest uniformly across sources.
//
// Returns "" (not null) when there's no description so callers can
// still hash-and-store consistently — empty-string and null mean the
// same thing semantically.
function extractCityDescription(detailJson) {
  const raw = detailJson?.content?.description;
  if (typeof raw !== "string" || !raw) return "";
  return raw
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/(p|div|li|h\d)>/gi, "\n")
    .replace(/<[^>]+>/g, " ")
    // Common HTML entities that show up in the CMS feed.
    .replace(/&nbsp;/g, " ")
    .replace(/&amp;/g, "&")
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    // Collapse runs of whitespace (including the newlines we just
    // inserted) into single spaces. Keeps the blob compact for the
    // Gemini prompt without losing readability of stored DB text.
    .replace(/\s+/g, " ")
    .trim();
}
...
function buildCityEventRow(lobbyEntry, detailJson) {
  ...
  // ensureLocationKey. Empty address (e.g. "רחבי העיר" / "כללי")
  // falls back to just the venue name; ensureLocationKey already
  // handles placeholder semantics.
  const rawAddress = venueAddress
  ...
  // Single-event registration URL. Same semantics as the
  // umbrella-child case (see buildCityChildEventRow): a non-NULL
  // value tells getBookingUrl to send the user straight to the
  // third-party provider instead of the city slug page. Smarticket
  // candidates were already filtered out by classifyDetail, so any
  // registerLink we see here is genuinely an external one we want
  // to honour.
  const externalUrl =
...
// ─────────────────────────────────────────────────────────────────
function buildCityChildEventRow(lobbyEntry, detailJson, child) {
  ...
  const externalUrl =
    typeof child?.registerLink === "string" && child.registerLink.trim()
      ? child.registerLink.trim()
      : null;
  ...
  return {
    ...
    external_url: externalUrl,
    // Track the umbrella relationship (sql/054). After fan-out the
    // parent row is intentionally NOT persisted (see
    // `deleteCityEventBySlug` in cityApiScraper.js — parent umbrellas
    // like shavuot-2026 aren't real events, just marketing pages), so
    // every child carries the parent fingerprint inline. Used by the
    // bot's `umb:<slug>` callback to render "📋 כל אירועי <title>"
    // listings.
