...
const ACCESS_RULES = [
  {
    ...
    match: /מוגבלות|מוגבלויות|צרכים\s*מיוחדים/,
  },
  {
    ...
    match: /הקהילה\s*הגאה|המרכז\s*הגאה|קהילה\s*גאה/,
  },
  {
    access: "community-seniors",
    // Rare via category, but reserved for future use.
    match: /הגיל\s*השלישי\s*בלבד|רק\s*ל\s*60\+/,
  },
];
...
function mapAccess(lobbyEntry, detailJson) {
...
function buildCityEventRow(lobbyEntry, detailJson) {
  ...
  // detail wins on the rare cases the lobby is stale.
  const imagePath =
    detailJson?.content?.media?.link || lobbyEntry.eventBackground?.link;
  ...
  return {
    ...
    // NOTE: we intentionally do NOT persist the prose description on
    // the row. The Gemini enrichment step (lib/eventEnricher.js,
    // rg-muni branch) re-fetches the city detail JSON on demand to
    // extract tags, and discards the text after. Keeping descriptions
    // out of `events` saves DB space — they can run several KB each
    // and aren't read by any other code path.
...
function buildCityChildEventRow(lobbyEntry, detailJson, child) {
  ...
  const childEndTime = computeEndTime(childHour, childDurationMinutes);
  ...
  // Name resolution: by default the child inherits the parent's
  // title ("every 'Active Garden' session IS an 'Active Garden'
  // event"; the venue varies in `location_key`). BUT for campaign
  // umbrellas like `shavuot-2026`, each schedule entry is a
  // semantically distinct event ("טנא מלא כוכבים", "חגיגת שבועות
  // בבורסה", "יריד שבועות") bundled under a shared marketing page.
  // Showing all of them as "שבועות ברמת גן" loses the user-facing
  // identity and makes the result list look duplicated.
  //
  // Heuristic: if the child carries its OWN non-empty title that
  // differs from the parent, use the child's. Otherwise — same
  // title or no title — inherit. This is backward-compatible with
  // the active-garden pattern (children have empty/identical titles
  // → still inherit) AND surfaces real names for campaign-style
  // umbrellas.
  const parentTitle =
  ...
  const eventName =
    rawChildTitle && rawChildTitle !== parentTitle ? rawChildTitle : parentTitle;
  ...
  // Image: parent's (children don't carry their own).
  const imagePath =
    detailJson?.content?.media?.link || lobbyEntry.eventBackground?.link;
  ...
  return {
    ...
    access,
    image: resolveImageUrl(imagePath),
    // See note in buildCityEventRow — descriptions are NOT persisted.
    // The enricher re-fetches the parent's detail JSON on demand and
    // throws the prose away after Gemini reads it.
    _rawAddress: rawAddress,
    _tagNames: extractTagNames(lobbyEntry, detailJson),
  };
}
