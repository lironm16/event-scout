...
const { normalizeScrapedAudience } = require("./seniorAudience");
...
const CITY_AUDIENCE_PRIORITY = [
  ...
  // 60+ city programming — distinct ENUM (sql/070).
  { match: /אזרחים\s*ותיקים|הגיל\s*השלישי|גיל\s*הזהב/, audience: "ותיקים" },
  { match: /צעירים|מבוגרים|18[\s\-–+]/, audience: "מבוגרים" },
];
...
function mapAccess(lobbyEntry, detailJson, opts = {}) {
  ...
  // Event title / description — many community events spell the audience
  // in the headline ("משרתי.ות מילואים") while the CMS category stays generic.
  if (opts.eventName) addAll(opts.eventName);
  addAll(lobbyEntry?.title);
  addAll(detailJson?.content?.title);
  const detailDesc =
    detailJson?.content?.description ||
    detailJson?.content?.info ||
    detailJson?.content?.body ||
    null;
  if (detailDesc) addAll(detailDesc);
...
function extractAudienceSubtypeTags(audienceTypeArray, eventName = "") {
  ...
  // Subtype tags (גיל הזהב / צעירים) only refine the generic adult tier.
  ...
  // Senior cohort is encoded via `normalizeScrapedAudience` → ותיקים
  // and `access: community-seniors` — not a "גיל הזהב" discovery tag.
  const out = [];
...
function buildCityEventRow(lobbyEntry, detailJson) {
  ...
  const access = mapAccess(lobbyEntry, detailJson, { eventName });
  const audience = normalizeScrapedAudience(mapAudience(lobbyEntry.audienceType), {
    name: eventName,
    access,
    audienceType: lobbyEntry.audienceType,
  });
  ...
  return {
    ...
    // Keep the city blurb even when there's an external registration link —
    // users want a description in-app (was previously nulled when external_url).
...
function buildCityChildEventRow(lobbyEntry, detailJson, child, umbrellaId = null) {
  ...
  return {
    ...
    description: childDescriptionText || null,
    _rawAddress: rawAddress,
    _tagNames: extractTagNames(lobbyEntry, detailJson),
  };
}
