...
const { formatKidProfileSuffix, kidAgeMonths } = require("./kidAge");
...
function kidManageButtonLabel(kid, index) {
  const name = String(kid?.name || `ילד/ה ${index + 1}`).trim();
...
function buildKidsManageKeyboard(kids, { editReturn = null } = {}) {
  ...
  rows.push([Markup.button.callback("➕ הוספת ילד/ה", "onb:kids:new")]);
...
function buildKidEditHubKeyboard(draft) {
  ...
  if (draft?.band === "0-3") {
    rows.push([Markup.button.callback("🧩 שלבי התפתחות", "onb:kid:field:stages")]);
  }
  rows.push([Markup.button.callback("✏️ שם", "onb:kid:field:name")]);
  if (draft?.editIndex != null) {
    rows.push([Markup.button.callback("🗑️ מחק ילד/ה", `onb:kid:del:${draft.editIndex}`)]);
  }
