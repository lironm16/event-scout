...
async function archivePastEvents() {
  ...
  // NOTE: we intentionally KEEP saved_events for archived events — a passed
  // bookmark is the signal that the user likely attended, and drives the
  // post-event review prompt (lib/reviewService + sendReviewPrompts). The Mini
  // App hides past saves from the saved list anyway (client-side notPast).
...
async function deleteAdminEntries() {
  // Match candidates with the REAL regexes via isAdminEntry — an ILIKE built
  // from a regex source can't honor anchors/word-boundaries and would
  // over-delete (e.g. "טסטוסטרון" for a /טסט/ substring). Fetch names, filter
  // precisely in JS, delete only true matches.
  const { data, error } = await supabase
    .from("events")
    .select("id, name")
    .eq("archived", false);
  if (error) {
    console.error("[Archive] Admin entry scan failed:", error.message);
    return { deleted: 0 };
  }
  const ids = (data || []).filter((e) => isAdminEntry(e.name)).map((e) => e.id);
  ...
  const { data: del, error: delErr } = await supabase
    ...
    .select("id, name");
  if (delErr) {
    console.error("[Archive] Admin entry deletion failed:", delErr.message);
    return { deleted: 0 };
  }
...
async function runCleanup() {
  ...
  // Archiving drops events from the ACTIVE set, so labels whose last active
  // event was just archived fall to events_count=0. The per-label trigger
  // prune (sql/050) fires only on DIRECT tag changes, not on the archive path
  // — so orphans accumulate. Recompute all counts + prune them here, then
  // clear the in-process label cache so a stale name→id mapping can't write a
  // deleted label id into a freshly-enriched event (sql/050 race note).
  let prunedLabels = 0;
  try {
    await supabase.rpc("refresh_label_counts"); // full recompute, counts only
    const { data } = await supabase.rpc("prune_zero_labels");
    prunedLabels = Number(data) || 0;
    if (prunedLabels) {
      try { require("./labelStore")._clearCache(); } catch { /* non-fatal */ }
    }
  } catch (err) {
    console.warn(`[Cleanup] label prune failed: ${err.message}`);
  }
  return { deleted, archived, prunedLabels };
}
