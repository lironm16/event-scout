...
//
// Identity tuple: `(name, min_months, max_months)`.
//   • `name` alone is usually enough — Hebrew event titles in this
//     dataset bake the age range into the title (e.g.
//     "...לגיל לידה עד שנה" vs "...לגיל שנה עד שלוש"), so two age
//     cohorts don't collide on name.
//   • Adding `(min_months, max_months)` is double-defense for any
//     case where the title is ambiguous about age.
...
function venueIdentity(occurrence) {
  ...
  if (occurrence.location_key) return `key:${occurrence.location_key}`;
  return "__no_key__";
}
