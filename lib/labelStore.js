...
// Strip a trailing year (4-digit or 2-digit) from a label name so that
// "שבועות 2026" and "שבועות 26" both normalise identically to "שבועות".
// City cluster names routinely append the current year as an editorial
// grouping hint, but from a user/tagging perspective these are the same
// concept as the base holiday/topic name.
function stripTrailingYear(s) {
  return s
    .replace(/\s+\d{4}$/, "") // e.g. "שבועות 2026" → "שבועות"
    .replace(/\s+\d{2}$/, "") // e.g. "שבועות 26"   → "שבועות"
    .trim();
}

function normalizeName(s) {
  if (!s) return null;
  return stripTrailingYear(stripBureaucraticSuffix(String(s)))
    .replace(/[\u0027\u0022\u05F3\u05F4\u2018\u2019\u201C\u201D`]/g, "")
    .replace(/\s+/g, " ")
    .trim()
...
// Curated canonical aliases — variant phrasings that normalizeName can't
// safely fold (the definite-article "ה", a "מחלקת …" department prefix) map
// to ONE canonical label, so Gemini emitting "הקהילה הגאה" /
// "מחלקת הקהילה הגאה" no longer spawns duplicates of "קהילה גאה". Blind
// ה-stripping is unsafe (it would corrupt words like "הצגה"/"הרצאה"), hence a
// curated list. Extend it when jobs/findDuplicateLabels.js surfaces a new
// cluster. Keyed by normalizeName(variant) → canonical display name.
const LABEL_ALIAS_PAIRS = [
  ["הקהילה הגאה", "קהילה גאה"],
  ["מחלקת הקהילה הגאה", "קהילה גאה"],
];
const LABEL_ALIASES = new Map(
  LABEL_ALIAS_PAIRS.map(([variant, canonical]) => [normalizeName(variant), canonical]),
);
...
const { isCoveredConcept } = require("./coveredConcepts");

async function getOrCreateLabel(rawName) {
  if (!rawName) return null;
  // First-pass cleanup: strip bureaucratic suffix and collapse whitespace.
  // "שבת תרבות - מחלקת תרבות-ר''ג" → "שבת תרבות"
  let cleaned = stripBureaucraticSuffix(
    String(rawName).replace(/\s+/g, " "),
  ).trim();
  if (!cleaned) return null;
  // Fold known variant phrasings to the canonical label (see LABEL_ALIASES)
  // so "הקהילה הגאה" / "מחלקת הקהילה הגאה" resolve to the same row as "קהילה גאה".
  const canonical = LABEL_ALIASES.get(normalizeName(cleaned));
  if (canonical) cleaned = canonical;

  // A concept owned by a dedicated field (audience value / access community)
  // must never become a label row — it would duplicate the structured field.
  // e.g. "נוער" → audience, "קהילה גאה" → access. Drop it (resolveMany skips null).
  if (isCoveredConcept(cleaned)) return null;
  ...
  // No mechanical match. Before minting a BRAND-NEW label, ask Gemini whether
  // this candidate is actually the SAME CONCEPT as an existing tag (catches
  // synonyms / phrasing the normalizer + alias map can't). Fail-open: any
  // error / rate-limit / disabled → null → we create the label below.
  try {
    const { canonicalizeNewLabel } = require("./labelCanonicalizer");
    // Only the IN-USE vocabulary (events_count > 0) — don't offer Gemini a
    // transient/empty label as a merge target (and keeps the list tight).
    const { data: rows } = await supabase
      .from("labels").select("id, name").gt("events_count", 0);
    const canon = await canonicalizeNewLabel(cleaned, (rows || []).map((r) => r.name));
    if (canon) {
      const match =
        (rows || []).find((r) => r.name === canon) ||
        (rows || []).find((r) => normalizeName(r.name) === normalizeName(canon));
      if (match) {
    ...
    /* fail-open → create the label below */
  }

  // Strip trailing year before persisting so "שבועות 2026" is stored as
  // "שבועות". normalizeName lowercases, so we replicate only the year-strip
  // step to preserve original casing for display.
  const storedName = cleaned.replace(/\s+\d{4}$/, "").replace(/\s+\d{2}$/, "").trim();

  const { data, error } = await supabase
    .from("labels")
    .insert({ name: storedName })
    .select("id")
    .maybeSingle();
  if (error) {
...
async function resolveTagNamesToIds(rawNames) {
  ...
  for (const raw of rawNames) {
    ...
    // Tier 1.25: playroom synonyms — "משחקייה" must NOT collapse to the
    // generic tag "משחק" (substring tier 2 would pick it by length).
    if (!hit && /משחקי/.test(norm)) {
      hit =
        tags.find((t) => t.name === "משחקיות") ||
        tags.find((t) => /משחקייה|משחקיה/.test(t.name || ""));
    }
    ...
    if (hit?.name === "משחק" && /משחקי/.test(norm)) {
      hit = tags.find((t) => t.name === "משחקיות") || null;
    }
