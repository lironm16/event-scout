...
const { formatTicketsLine, formatLowStockBadge, buildNavButtons } = require("./eventCard");
const {
...
// used by the interactive search, so the user sees consistent
// walk/drive labels in both flows.
async function proximityMatches(event, enriched, filters, profile) {
  ...
  const result = await evaluateProximity(
    home,
    event.location || null,
    profile?.user_context?.constraints?.max_walking_minutes || null,
  ...
  if (pref === "walk") return !result.requires_car;
  if (pref === "drive") return result.requires_car;
  return true;
}
...
function buildKeyboard(event, savedSearch) {
  ...
  const navRow = buildNavButtons(event);
  ...
  for (const { ss, profile, events: matches } of matchesBySearch.values()) {
    ...
    try {
      if (matches.length === 1) {
        ...
        const reply_markup = buildKeyboard(event, ss);
