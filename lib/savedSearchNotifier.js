...
const { audienceVerdict, deriveDefaultAudienceSet, ageMatches } = require("./categories");
...
function audienceMatchesSavedSearch(eventName, filters, enriched, profile) {
  ...
  return allowed.has(enriched.audience);
}
...
function buildMatchMessage(event, savedSearch, profileFirstName) {
  ...
  if (event.tickets_left) lines.push(`🎫 ${event.tickets_left} כרטיסים`);
...
function buildBatchMessage(events, savedSearch, profileFirstName) {
  ...
  for (const ev of events) {
    ...
    if (ev.tickets_left) lines.push(rtlLine(`   🎫 ${ev.tickets_left} כרטיסים`));
    lines.push(``);
  }
