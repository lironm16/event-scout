...
async function buildProfileEditPayload(profile) {
  ...
  return {
    profile: {
      ...
      // membership, applyProfilePatch catches it at save (lib/conceptConflict).
...
async function applyProfilePatch(telegramId, patch = {}) {
  ...
  // Conflict guard (model א): a suppressed tag-label that names an audience the
  // user positively targets, or a community they belong to, is a contradiction.
  // Detect on the FINAL post-patch state and refuse to persist anything — the
  // Mini App explains it and forces the user to resolve before re-saving.
  {
    const ctx = existing?.user_context || {};
    const existingSupp = Array.isArray(ctx.suppressed_labels) ? ctx.suppressed_labels : [];
    const adds = (patch.add_suppressed_labels || [])
      .filter((s) => typeof s === "string" && s.trim())
      .map((s) => s.trim());
    const removes = new Set((patch.remove_suppressed_labels || []).map((s) => normConflict(s)));
    const finalSupp = [...new Set([...existingSupp, ...adds])].filter(
      (s) => !removes.has(normConflict(s)),
    );
    const finalChips = Array.isArray(patch.audience_chip_ids)
      ? patch.audience_chip_ids
      : [...hydrateTargetAudienceChipIds(existing)];
    const finalCommunities =
      patch.communities && typeof patch.communities === "object"
        ? patch.communities
        : ctx.communities || {};
    const conflicts = detectProfileConflicts({
      suppressedNames: finalSupp,
      audienceChipIds: finalChips,
      communities: finalCommunities,
    });
    if (conflicts.length) throw new ProfileConflictError(conflicts);
  }
