...
const { STAGES: DEV_STAGE_DEFS, LEVELS: DEV_LEVELS, sanitizeKidDevStages } = require("./devStages");
...
// Resolve suppressed venue KEYS → [{ key, name }] via the locations table so
// the profile shows readable venue names instead of raw location keys.
async function resolveSuppressedLocations(keys) {
  ...
  try {
    const { data } = await supabase
      ...
      .in("key", arr);
    for (const r of data || []) if (r.display_name) names[r.key] = r.display_name;
  } catch { /* fall back to keys */ }
  return arr.map((k) => ({ key: k, name: names[k] || k }));
}

async function buildProfileEditPayload(profile) {
  const ctx = profile?.user_context || {};
  const constraints = ctx.constraints || {};
  const interests = Array.isArray(ctx.interests) ? ctx.interests : [];
  const suppressedLocations = await resolveSuppressedLocations(
  ...
  return {
    profile: {
      ...
      kids: (Array.isArray(ctx.kids) ? ctx.kids : []).map((k) => ({
        ...
        // Per-stage developmental readiness { stage: level } (sanitized).
        dev_stages: sanitizeKidDevStages(k?.dev_stages),
      })),
      topic_ids: topicIds,
      ...
      // Show ALL suppressed labels — including audience/community ones. A user
      // CAN suppress them; if that contradicts a positive audience/community
      // membership — tag suppression is purely mechanical.
      ...
      suppressed_locations: suppressedLocations,
    },
    options: {
      genders: GENDERS,
      devStages: DEV_STAGE_DEFS, // [{id,label,fromM,toM}] — age-relevant filtering client-side
      devLevels: DEV_LEVELS,     // [{id,label}] — 4 readiness levels
...
function sanitizeKids(kids) {
  ...
  return kids
    .map((k) => {
      ...
      const dev_stages = sanitizeKidDevStages(k?.dev_stages);
      return { name, birth_date, gender, dev_stages };
    })
...
async function applyProfilePatch(telegramId, patch = {}) {
  ...
  // Tag suppression is purely mechanical — if a user suppresses a tag, all
  // events carrying that tag are hidden, regardless of what the tag name means
  // semantically (community, audience, content). No conflict detection.
  ...
  // Suppress ONE audience ("אל תראה לי יותר → לפי קהל"). Audience chips ARE the
  // audience_t enum values, so removing it from the positive set hides exactly
  // that audience — consistent with the profile/search model. When the user
  // had no chips (sees all), start from all 7 then drop the one.
  if (typeof patch.remove_audience === "string" && AUDIENCE_CHIP_IDS.has(patch.remove_audience)) {
    const prof = await getProfile(telegramId);
    const current = prof?.user_context?.target_audience_chip_ids;
    const base = (Array.isArray(current) && current.length) ? current : [...AUDIENCE_CHIP_IDS];
    const next = base.filter((id) => id !== patch.remove_audience && AUDIENCE_CHIP_IDS.has(id));
    await saveTargetAudienceChips(telegramId, next, { getProfile });
  }
  ...
  return await buildProfileEditPayload(await getProfile(telegramId));
}
