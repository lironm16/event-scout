...
async function buildProfileEditPayload(profile) {
  ...
  return {
    profile: {
      ...
      // Audience labels (ילדים/נוער/תינוקות/…) are managed ONLY by the positive
      // "🎯 קהלי יעד" selector — never surface them in the suppression list too
      // (redundant, and would conflict with a positive pick). Topical tags CAN
      // still be suppressed, so only audience labels are stripped here.
      suppressed_labels: (Array.isArray(ctx.suppressed_labels) ? ctx.suppressed_labels : [])
        .filter((l) => !AUD_LABELS.has(l)),
