...
const { setSuppressOnlineEvents, removeSuppressedLabel } = require("./tagSuppressPrefs");
...
const TIME_SLOTS = [
  ...
  { id: "afternoon", label: 'אחה"צ', start: "15:00", end: "18:00" },
  { id: "evening", label: "ערב", start: "18:00", end: "22:00" },
];

const LOCATION_MODES = [
  { id: "walk", label: "ברגל" },
  { id: "drive", label: "נסיעה קצרה" },
  { id: "any", label: "כל מקום" },
];
...
const AGE_RANGES = [
  { id: "young_adult", label: "צעיר/ה (18-35)" },
  { id: "mid_adult", label: "מבוגר/ת (35-60)" },
  { id: "senior", label: "ותיק/ה (60+)" },
];
...
function sanitizeKids(kids) {
  ...
  return kids
    ...
    .filter((k) => k.name || k.birth_date || k.stages.length);
}
...
async function applyProfilePatch(telegramId, patch = {}) {
  ...
  if ("age_range" in patch) brain.age_range = patch.age_range || null;
  if ("kids" in patch) brain.kids = sanitizeKids(patch.kids);

  // Topics only — saveTargetAudienceChips re-adds audience labels below.
  if (Array.isArray(patch.topic_ids)) {
    brain.interests = patch.topic_ids
  ...
  // 4) Removals (suppressions are added via feedback, removed here).
