...
async function getWatchedEvents(telegramId) {
  const { data, error } = await supabase
    .from("event_watchers")
    .select(
      "event_id, created_at, tickets_needed, events(id, name, date, start_time, end_time, image, tickets_left, location_key, locations:location_key(raw_address, lat, lng, found))"
    )
    .eq("telegram_id", String(telegramId))
