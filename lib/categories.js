...
// Developmental-stage matching — structured, NO regex. The event's targeted
// stages live in events.dev_stages (a SET of canonical ids set by the enricher
// from the LLM). The kid's stages live in the profile (canonical ids too). We
// match by exact id overlap. A legacy profile that stored the Hebrew LABEL
// instead of the id is folded via an EXACT label→id map (no pattern matching).
const { DEV_STAGES: _DEV_STAGES } = require("./kidsWizardUi");
const DEV_STAGE_IDS = new Set(_DEV_STAGES.map((s) => s.id));
const DEV_LABEL_TO_ID = new Map(_DEV_STAGES.map((s) => [s.label, s.id]));
function kidStageIdSet(kid) {
  const out = new Set();
  for (const s of kid?.stages || []) {
    const str = String(s);
    if (DEV_STAGE_IDS.has(str)) out.add(str);             // canonical id
    else if (DEV_LABEL_TO_ID.has(str)) out.add(DEV_LABEL_TO_ID.get(str)); // exact label
  }
  return out;
}
/** True when a household kid's DEV STAGE matches the event's targeted stages. */
function kidStageFitsEvent(event, kidsProfiles) {
  if (!Array.isArray(kidsProfiles) || !kidsProfiles.length) return false;
  const evStages = Array.isArray(event?.dev_stages) ? event.dev_stages : [];
  if (!evStages.length) return false;
  const evSet = new Set(evStages);
  for (const kid of kidsProfiles) {
    for (const id of kidStageIdSet(kid)) if (evSet.has(id)) return true;
  }
  return false;
}
