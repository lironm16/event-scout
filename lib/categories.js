...
function deriveDefaultAudienceSet(profile) {
  ...
  // - `age_range` senior  → `ותיקים` (60+ programming).
  // - `age_range` young/mid → `מבוגרים`; subtype tag gate still
  //   narrows young_adult vs senior-tagged rows inside that tier.
  ...
  const suppressKids = Boolean(profile?.user_context?.suppress_child_audiences);
  if (hasKids && !suppressKids) {
...
function eventMatchesActivityTypes(event, requestedTypesEN) {
  ...
  for (const t of requestedTypesEN) {
    ...
    if (t === "party") {
      if (event?.category && categoryMatches(event, [t])) return true;
      continue;
    }
