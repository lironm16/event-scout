...
function deriveDefaultAudienceSet(profile) {
  ...
  const hasAgeRange = ageRange === "young_adult" || ageRange === "mid_adult" || ageRange === "senior";

  // Two INDEPENDENT dimensions (May-2026 redesign). A young parent
  // (kids:true, age_range:'young_adult') wants BOTH family/kids
  // events AND their own young-adult-tier adults events. The old
  // single-dimension default ("kids OR adults, never both") forced
  // them to manually opt into "תראי לי הכל" for date-night content.
  //
  // - `לכל המשפחה` is ALWAYS allowed — it's the "for everyone"
  //   tier the city CMS uses for community events that don't pin to
  //   a single demographic.
  // - `kids[]` populated → add the kid tiers (babies/kids/teens).
  // - `age_range` set    → add `מבוגרים` (the user's own adult tier).
  //   The per-subtype tag filter (`deriveExcludedSubtypeTags`) then
  //   biases WHICH `מבוגרים` events show: young filter out senior-
  //   tagged content; senior filters out young-tagged. Mid-adults
  //   gets the unfiltered `מבוגרים` pool.
  // - Neither dimension set (truly empty profile, no setup done) → show
  //   everything. A user who hasn't configured their profile shouldn't
  //   see an empty catalog or miss the "online" filter chip because their
  //   age_range/kids haven't been filled in yet.
  const allowed = new Set(["לכל המשפחה"]);
  if (hasKids) {
    allowed.add("תינוקות").add("ילדים").add("נוער");
  }
  if (hasAgeRange) {
    allowed.add("מבוגרים");
  }
  if (!hasKids && !hasAgeRange) {
    // No profile info at all — include all tiers so the catalog is fully
    // browsable. Once the user completes profile setup the correct subset
    // is applied automatically.
    allowed.add("מבוגרים").add("תינוקות").add("ילדים").add("נוער");
  }
  return allowed;
}
...
function deriveExcludedSubtypeTags(profile) {
  const ageRange = profile?.user_context?.age_range || profile?.age_range || null;
  if (ageRange === "young_adult") return ["גיל הזהב"];
