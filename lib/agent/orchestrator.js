...
const { buildSystemPrompt, buildProfileContext, STATIC_SYSTEM_INSTRUCTION } = require("./systemPrompt");
...
const { GEMINI_MODEL: MODEL } = require("../geminiModel");

// ── Context caching ────────────────────────────────────────────────────────
// The static system instruction (~49 K chars / ~12 K tokens) is identical for
// every user and every turn. By caching it at the Gemini API level, subsequent
// calls pay the cached-token rate (≈75 % cheaper) instead of the full rate.
//
// Cache TTL: 2 hours. The cache auto-refreshes lazily before it expires.
// If the API rejects caching (quota, model support, etc.) we fall back
// transparently to the non-cached path — the agent still works, just costs a
// bit more until the next restart.
// ──────────────────────────────────────────────────────────────────────────
const cacheManager = new GoogleAICacheManager(process.env.GEMINI_API_KEY);
const CACHE_TTL_SECS = 7_200; // 2 hours
let _cachedContentName = null; // resource name returned by the API
let _cacheExpiresAt = 0;       // ms timestamp when our local record expires

async function getOrCreateCache() {
  const now = Date.now();
  if (_cachedContentName && now < _cacheExpiresAt) return _cachedContentName;

  const cache = await cacheManager.create({
    model: `models/${MODEL}`,
    displayName: "event-scout-agent-static",
    systemInstruction: {
      parts: [{ text: STATIC_SYSTEM_INSTRUCTION }],
    },
    tools: toolsConfig,
    ttlSeconds: CACHE_TTL_SECS,
  });
  _cachedContentName = cache.name;
  // Subtract 2-minute buffer so we refresh before the API rejects us.
  _cacheExpiresAt = now + (CACHE_TTL_SECS - 120) * 1_000;
  console.log(`[Agent] Context cache created: ${_cachedContentName}`);
...
async function runAgent(telegramId, ctx) {
  if (!isGeminiAllowed("agent")) {
    console.log("[Agent] Gemini disabled (GEMINI_ONLY_ENRICHER) — skipping runAgent");
    if (ctx?.reply) {
      await ctx.reply("הסוכן כבוי כרגע. השתמשי ב-/search או בכפתורי החיפוש.").catch(() => {});
    }
    return;
  }
  ...
  // Build model — try context-cached path first, fall back to non-cached.
  let model;
  ...
  try {
    ...
    console.warn("[Agent] Context cache unavailable, falling back to full prompt:", cacheErr.message);
  ...
  // When using the cached static instruction, the profile/date context is NOT
  // inside the system instruction anymore. Inject it as a synthetic first
  // exchange prepended to every conversation so the model always has fresh
  // date and profile data without it counting against the cache hit.
  const profilePrefix = usingCache
    ? [
        { role: "user",  parts: [{ text: buildProfileContext({ profile }) }] },
        { role: "model", parts: [{ text: "מבינה." }] },
      ]
    : [];
  ...
  return await tracedAgentTurn(
    ...
    { ctx, session, model, telegramId, traceId, profilePrefix },
  );
}

const tracedAgentTurn = trace(
  async function agent_turn(_inputs, runtime) {
    const { ctx, session, model, telegramId, traceId, profilePrefix = [] } = runtime;
    let rounds = 0;
    ...
    while (true) {
      ...
      let result;
      try {
        // Prepend the profile/date context block when using context caching.
        // profilePrefix is [] on the non-cached path (no-op in that case).
        const contents = profilePrefix.length
          ? [...profilePrefix, ...session.history]
          : session.history;
        result = await withTimeout(
          tracedGenerate({ contents }, model),
        ...
        // Transient overload (Gemini 503 / high-demand): retry the
        // same round up to 2 times with a short back-off before
        // giving up. The user's message is still in session.history
        // so no content is lost.
        const isOverload =
          /503|service unavailable|high demand|overload/i.test(err.message);
        const overloadKey = `overload_retries_r${rounds}`;
        const overloadRetries = (session[overloadKey] || 0);
        if (isOverload && overloadRetries < 2) {
          session[overloadKey] = overloadRetries + 1;
          const backoffMs = 2000 * (overloadRetries + 1); // 2s, 4s
          console.warn(
            `[Agent] round ${rounds} — Gemini overload, retry ${overloadRetries + 1}/2 in ${backoffMs}ms`,
          );
          await new Promise((r) => setTimeout(r, backoffMs));
          rounds--;   // re-run this round
          continue;
        }
