  ...
  async function agent_turn(_inputs, runtime) {
    ...
    while (true) {
      ...
      if (!calls.length) {
        ...
        // forbids this, but if it happens we surface it to the user rather
        // than swallow the response.
        const text = extractText(response);
        if (text) {
          await ctx.tg.reply(text).catch(() => {});
          tracing.setOutput(traceId, text);
          sessionStore.appendModelTurn(telegramId, modelParts.length ? modelParts : [{ text }]);
        }
        return { ok: true, rounds, terminator: "raw_text", output: text };
      }
