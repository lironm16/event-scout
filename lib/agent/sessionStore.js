...
function ensureSession(telegramId) {
  ...
  if (!s) {
    s = {
      ...
      // Set<event_id> of events we've ALREADY rendered cards for in
      // this session. Populated by present_event_results, consumed by
      // search_events to exclude already-seen ids from new result
      // pages. Why: without this, "events this week" and "family
      // events" both legitimately match an event tagged as both, and
      // the user sees the same card twice across consecutive turns.
      // Bounded growth via FIFO eviction at SHOWN_EVENT_IDS_CAP. We
      // use an insertion-ordered Set (default JS) so the eviction is
      // free.
