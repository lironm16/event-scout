...
// Events rendered (in a «חיפוש כללי») as OUTSIDE the user's profile — so
// their card hid "אל תראה לי יותר" and skipped the "מתאים" badge. We
// remember the decision so a later "קרא עוד" re-render reproduces the
// SAME card instead of recomputing (and risking a different verdict on a
// leaner event object).
function rememberOutOfProfileEvents(telegramId, eventIds) {
  if (!Array.isArray(eventIds) || !eventIds.length) return;
  const s = ensureSession(telegramId);
  if (!s.outOfProfileEventIds) s.outOfProfileEventIds = new Set();
  for (const raw of eventIds) {
    const id = typeof raw === "number" ? raw : parseInt(raw, 10);
    if (!Number.isFinite(id)) continue;
    s.outOfProfileEventIds.delete(id);
    s.outOfProfileEventIds.add(id);
  }
  while (s.outOfProfileEventIds.size > SHOWN_EVENT_IDS_CAP) {
    const oldest = s.outOfProfileEventIds.values().next().value;
    if (oldest === undefined) break;
    s.outOfProfileEventIds.delete(oldest);
  }
}

function isOutOfProfileEvent(telegramId, eventId) {
  const s = sessions.get(key(telegramId));
  if (!s?.outOfProfileEventIds) return false;
  const id = typeof eventId === "number" ? eventId : parseInt(eventId, 10);
  return s.outOfProfileEventIds.has(id);
}
