    ...
    properties: {
      ...
      audience: {
        ...
        description:
          ...
          "Values: kids/family/adults/teens/toddlers/parents — explicit subset; matched against the event's `audience` ENUM column (sql/032). " +
...
async function searchEventsTool(args, ctx) {
  ...
  const _searchProfile = ctx?.telegramId
    ...
    : null;
  const accessScopes = accessScopesForProfile(_searchProfile);
  ...
  async function runFilterChain(searchFrom, searchTo) {
    ...
    if (suppressedLocations.length && !wantKey) {
  ...
  return {
    ...
    // Communities with events in this window that were silently filtered
    // because the user's status is unknown (neither 'member' nor
    // 'not-member'). Only non-zero counts are included. The agent should
    // mention these briefly in intro_text so the user knows content was
    // filtered — but should NOT proactively offer membership here (that's
    // the HYBRID FLOW's job, triggered by user interest signals).
    communities_excluded: await (async () => {
      // Friendly Hebrew label for each community key.
      const COMMUNITY_LABELS = {
        "community-seniors":      "הרצאות לאזרחים ותיקים",
        "community-miluim":       "אירועי מילואים",
        "community-lgbtq":        "אירועי הקהילה הגאה",
        "community-disabilities": "אירועים לאנשים עם מוגבלות",
        "community-russian":      "אירועים בקהילה הרוסית",
      };
      // Only probe communities whose status is unknown — members already
      // see the events; opted-out users don't need reminders.
      const unknownCommunities = COMMUNITY_ENUM.filter((c) => {
        const s = _userCommunities[c] ?? null;
        return s !== "member" && s !== "not-member";
      });
      if (!unknownCommunities.length) return [];
      // Parallel COUNT-only HEAD requests — no rows returned, just counts.
      const counts = await Promise.all(
        unknownCommunities.map((c) =>
          supabase
            .from("events")
      ...
      const result = [];
      for (let i = 0; i < unknownCommunities.length; i++) {
        const n = counts[i].count ?? 0;
        if (n > 0) {
          result.push({
            community: unknownCommunities[i],
            count: n,
            label_he: COMMUNITY_LABELS[unknownCommunities[i]] || unknownCommunities[i],
          });
        }
      }
      return result;
    })().catch(() => []), // never let community probes break the main response
  };
}
