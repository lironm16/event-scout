...
async function searchEventsTool(args, ctx) {
  ...
  async function runFilterChain(searchFrom, searchTo) {
    ...
    if (applyCohortGate) {
      ...
      const kept = [];
      for (const e of events) {
        if (shouldExcludeAdultSubtypeEvent(e, ctx.profile)) {
          audienceDropped.push(e);
          continue;
        }
        if (userAgeWindow && e.audience === "מבוגרים" && !ageWindowOverlaps(e, userAgeWindow)) {
          audienceDropped.push(e);
          continue;
        }
        kept.push(e);
    ...
    if (ageFilter != null) {
      ...
      const kidAges = Array.isArray(kids)
        ? kids.map((k) => k.age).filter((a) => a != null && Number.isFinite(a))
        : [];
      if (kidAges.length) {
        events = events.filter((e) => householdKidsFitEvent(e, kidAges));
      }
    }
    if (profileSuppressesChildEvents(ctx.profile)) {
    ...
    // Per-event "לא מתאים" — any reason hides this event_id for this user.
    if (ctx?.telegramId) {
