    ...
    properties: {
      ...
      communities: {
        ...
        description:
          ...
          "'community-lgbtq', 'community-seniors', 'community-miluim', " +
          "'community-russian', 'community-olim'. Values: 'member' " +
          ...
          "its audiences step (🏳️‍🌈 קהילה גאה / 🌷 ותיקים / 🧩 צרכים מיוחדים / 🎖️ משרתי " +
          "מילואים / 🇷🇺 דוברי רוסית / ✈️ עולים חדשים) writes directly into this field. Use this tool only when the user " +
          ...
          "User says 'אני דובר/ת רוסית' or volunteers Russian-community membership → " +
          "{ 'community-russian': 'member' }. " +
          "User says 'אני עולה חדשה' / 'אירועים לעולים' → " +
          "{ 'community-olim': 'member' }.",
        properties: {
...
//
// Persists explicit preference signals captured during conversation.
// Each adjustment names a tag, category, or recurring series and assigns
// a weight preset (boost/suppress/neutral etc.).  The tool merges the
// adjustments into user_context.preferences — it does NOT overwrite
// unrelated weights.
// ─────────────────────────────────────────────────────────────────────────
const updateUserPreferencesDecl = {
  name: "update_user_preferences",
  description:
    "Persist explicit user preference signals: things the user wants to see MORE or LESS of in search results. " +
    "Call this when the user makes an explicit verbal statement such as " +
    "'I'm not interested in X', 'show me more Y', 'skip Z', etc. " +
    "Do NOT call speculatively — only on clear, unambiguous user intent.\n\n" +
    "Weight presets:\n" +
    "  'strong_boost' (4×) / 'boost' (2×) — user actively wants more\n" +
    "  'neutral' (1×) — reset / remove preference\n" +
    "  'suppress' (0.2×) / 'strong_suppress' (0.05×) — user wants less\n\n" +
    "kind values:\n" +
    "  'tag'      — a label/tag the event carries (use the Hebrew label name)\n" +
    "  'category' — an event category string (e.g. 'הפעלה', 'מוזיקה')\n" +
    "  'series'   — a recurring series name to sink (only suppresses/un-suppresses)",
  parameters: {
    type: SchemaType.OBJECT,
    properties: {
      adjustments: {
        type: SchemaType.ARRAY,
        description: "List of preference adjustments to apply.",
        items: {
          type: SchemaType.OBJECT,
          properties: {
            kind: {
              type: SchemaType.STRING,
              enum: ["tag", "category", "series"],
              description: "Which dimension to adjust",
            },
            key: {
              type: SchemaType.STRING,
              description:
                "The value to adjust — Hebrew tag name, category string, or series name",
            },
            preset: {
              type: SchemaType.STRING,
              enum: [
              ...
              description: "How strongly to boost or suppress",
            },
          },
          required: ["kind", "key", "preset"],
        },
      },
    },
...
async function updateUserPreferences({ adjustments }, ctx) {
  if (!adjustments || !adjustments.length) {
    return { ok: false, reason: "No adjustments provided" };
  }
  await updatePreferences(ctx.telegramId, adjustments);
  const summary = adjustments.map((a) => `${a.kind}:${a.key}=${a.preset}`).join(", ");
  return { ok: true, applied: adjustments.length, summary };
}

module.exports = {
  declarations: [getUserProfileDecl, updateProfileDecl, updateUserPreferencesDecl],
  handlers: {
