...
const { selectSeriesForRender, venueIdentity, groupIntoSeries } = require("../../eventSeries");
...
const presentEventResultsDecl = {
  ...
  description:
    ...
    "A 'series' = events sharing the same name+venue+age tier ('משחקיית רגעים לגיל לידה עד שנה' " +
    "running 4× per week is ONE series). All occurrences of a series collapse into ONE card with " +
    "a 'כל המופעים' button. " +
...
async function presentEventResults(args, ctx) {
  ...
  for (const s of series) {
    ...
    try {
      ...
      if (s.occurrences.length > 1 && ctx.telegramId) {
  ...
  if (rendered > 0 && hits.length && ctx.telegramId) {
    ...
    if (moreRemainingSeries > 0) {
      ...
      const text = moreRemainingSeries === 1
        ...
        : `יש עוד ${moreRemainingSeries} סדרות — להראות?`;
      try {
      ...
      filters: {
        ...
        properties: {
          ...
          audience: {
            ...
            nullable: true,
            description:
              "Audience filter ENUM — one of ['תינוקות','ילדים','נוער','לכל המשפחה','מבוגרים','אזרחים ותיקים']. " +
