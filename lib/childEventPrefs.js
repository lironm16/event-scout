...
function profileSuppressesChildEvents(profile) {
  // DISABLED — the standalone "hide kids/babies/youth events" flag was redundant
  // with the positive "🎯 קהלי יעד" audience selector (the single source of truth
  // for which audiences a user sees). Always false now, so a stale
  // suppress_child_audiences value in any profile is ignored. (UI + payload
  // removed; full teardown of the flag/setters is a follow-up.)
  return false;
}
...
async function setSuppressChildAudiences(telegramId, suppress) {
  ...
  const chipIds = (ctx.target_audience_chip_ids || []).filter(
    (id) => !["ילדים", "תינוקות", "נוער"].includes(id),
  );
  await supabase
    .from("profiles")
    .update({
...
async function appendKidToProfile(telegramId, kid) {
  ...
  const entry = {
    name: kid.name,
    ...(kid.birth_date ? { birth_date: kid.birth_date } : {}),
