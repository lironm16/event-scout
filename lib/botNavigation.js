...
function searchHubKeyboard() {
  const { buildSearchDraftKeyboard } = require("./searchDraftPicker");
  return buildSearchDraftKeyboard();
}
...
function searchHubIntroText(draftText) {
  const { buildSearchDraftHeader, emptyDraft } = require("./searchDraftPicker");
  return buildSearchDraftHeader(emptyDraft(), draftText);
}
...
async function showMainMenu(ctx, { draftText = null } = {}) {
  ...
  await ctx.reply(mainMenuIntroText(draft), {
    parse_mode: "Markdown",
