...
function formatAgeRangeLabel(ageRange) {
  ...
  // A range whose BOTH bounds are whole years reads better as digits — "3-6",
  // not "שלוש עד שש" (matches the numeric formatter elsewhere).
  const wholeYears = (ep) => {
    if (!ep || ep.kind === "stage" || ep.inclusive === false) return null;
    if (ep.kind === "years" && Number.isInteger(ep.value) && ep.value >= 1) return ep.value;
    if (ep.kind === "months" && ep.value >= 12 && ep.value % 12 === 0) return ep.value / 12;
    return null;
  };
  if (minL && maxL) {
    const my = wholeYears(ar.min), My = wholeYears(ar.max);
    if (my != null && My != null) return `${my}-${My}`;
    return `${minL} עד ${maxL}${tail(ar.max)}`;
  }
