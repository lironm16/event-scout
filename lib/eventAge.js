...
function sanitizeEndpoint(ep) {
  ...
  let value = ep.value;
  if (kind === "stage") {
    if (!(value in STAGE_MONTHS)) return null;
  } else {
...
/** One endpoint → months (stage map / years×12 / months as-is), or null. */
function endpointMonths(ep) {
  if (!ep) return null;
  if (ep.kind === "stage") return STAGE_MONTHS[ep.value] ?? null;
...
function endpointLabel(ep) {
  ...
  if (ep.kind === "years") return YEAR_WORD[ep.value] || `${ep.value} שנים`;
  if (ep.kind === "months") return ep.value === 1 ? "חודש" : `${ep.value} חודשים`;
  return null;
}
