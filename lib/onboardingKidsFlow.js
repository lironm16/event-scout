...
function buildOnboardingKidsKeyboard(state) {
  ...
  if (step === "kids_birthdate") {
    return { inline_keyboard: buildBirthDateBackKeyboard("onb", null) };
  }
  if (step === "kids_stages") {
    return {
  ...
  if (step === "kids_gender") {
    return { inline_keyboard: buildKidGenderKeyboard("onb", null) };
  }
  if (step === "kids_more") {
    return {
      inline_keyboard: [
...
function registerOnboardingKidsHandlers(bot, deps) {
  ...
  bot.action(/^onb:kgen:(male|female|skip)$/, async (ctx) => {
    ...
    sessionStore.updateOnboarding(telegramId, { kidsDraft: d, step: "kids_name" });
    await renderOnboardingStep(ctx, sessionStore.getOnboarding(telegramId));
  });
