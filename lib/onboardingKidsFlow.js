...
function commitKidsDraft(state) {
  ...
  // Developmental readiness is set in the Mini App profile (per-stage 4-level)
  // and otherwise derived from age — the bot wizard no longer collects the
  // legacy `stages` array.
  const entry = {
    name: d.name,
...
function buildOnboardingKidsBody(state, gender) {
  ...
  if (step === "kids_manage") {
    ...
    return (
      "👧 *הילדים שלך*\n\n" +
      (n
        ? `יש ${n} ילדים ברשימה. לעריכה — לחצי על השורה. אפשר גם להוסיף או למחוק.`
        : "אין עדיין ילדים ברשימה — הוסיפי ילד.")
    );
  }
  if (step === "kids_edit") {
    const d = state.kidsDraft;
  ...
  if (step === "kids") {
    return (
      ...
      "• תינוק (0–3) — תאריך לידה, שלבים, מגדר, שם\n" +
      "• ילד (3–18) — תאריך לידה, מגדר, שם\n" +
      "• אפשר כמה ילדים"
    );
  }
  if (step === "kids_birthdate") {
    return birthDatePromptForKid(state.kidsDraft?.gender);
  }
  if (step === "kids_stages") {
    return kidsStagesStepBody(state.kidsDraft?.gender, mark);
  }
  if (step === "kids_gender") {
    return `⚧ *מגדר ${kidNounDefiniteHe(state.kidsDraft?.gender)}*\n\nבן, בת, או «לא מציין»:`;
  }
  if (step === "kids_name") {
    return kidNameStepBody(state.kidsDraft?.gender);
  }
  if (step === "kids_more") {
...
function buildOnboardingKidsKeyboard(state) {
  ...
  if (step === "kids_more") {
    return {
      inline_keyboard: [
        [
          Markup.button.callback("כן, עוד ילד", "onb:kmore:y"),
...
async function handleOnboardingKidsText(ctx, message, sessionStore, { renderOnboardingStep }) {
  ...
  if (state.step === "kids_birthdate" && state.kidsDraft?.awaitingBirthDate) {
    ...
    // Skip the legacy kids_stages step — developmental readiness is now set in
    // the Mini App (per-stage 4-level) / derived from age. Go straight to gender.
