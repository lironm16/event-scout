...
// Emoji + label per audience value.
const AUDIENCE_ICON = {
  "הורים":        "👪",
  "נוער":         "🧑",
  "ילדים":        "👧",
...
function formatAudienceLine(event) {
  ...
  if (!event.audience) {
    ...
    if (range && (effMin != null || effMax != null)) return `👥 ${range}`;
  ...
  if (event.audience === "לכל המשפחה") {
    ...
    if (range && (effMin != null || effMax != null)) {
      if (PARENTING_RE_FORMAT.test(event.name || "")) return `👪 ${range}`;
      return `${icon} ${range}`;
    }
    if (PARENTING_RE_FORMAT.test(event.name || "")) return "👪 להורים";
