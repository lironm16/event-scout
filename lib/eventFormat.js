...
const TOPIC_RULES = [
  ...
  { icon: "📚", patterns: [/סיפור/, /ספרים/, /ספרי[יה]/, /ספרון/, /\bbook\b/i, /\bbooks\b/i, /\bstory\b/i, /\bstories\b/i, /קריאה/, /ספרות/] },
  ...
  // Fairs / markets — low priority so a *book* fair still resolves to 📚 above.
  { icon: "🛍️", patterns: [/יריד/, /\bשוק\b/, /באזар/, /\bfair\b/i, /\bmarket\b/i, /\bbazaar\b/i] },
];

function getEventIcon(event) {
  // Prefer the STRUCTURED emoji the enricher (Gemini) chose. The TOPIC_RULES
  // regex below is only a fallback for rows enriched before emoji was stored
  // (event.emoji null/empty) — it retires once emoji is backfilled.
  if (typeof event?.emoji === "string" && event.emoji.trim()) return event.emoji.trim();
...
function isCanonicalFamilyAgeRange(min, max) {
  // "All ages" sentinel — a floor of 0 OR null with an open/100y top. We accept
  // null min too: the enricher/backfill sometimes stores max=1200 with min=null
  // for an all-ages event, which must still read as "לכל המשפחה", not "לידה עד 100".
  const openFloor = min == null || min === 0;
  const openTop = max == null || max >= OPEN_AGE_MAX_MONTHS;
  return openFloor && openTop;
}
...
// Parent-focused = the STRUCTURED audience the enricher set. No regex text-scan
// (הריון/לידה/הנקה/… events are classified audience="הורים" by Gemini).
function isParentFocusedEvent(event) {
  return event?.audience === "הורים";
}
...
// grade = the parsed grade-range string; isParent = structured audience==="הורים".
function formatGradeAudienceLabel(grade = null, isParent = false) {
  if (!grade) return null;
  return isParent ? `הורי ${grade}` : grade;
}

// Parent audience line — driven by the STRUCTURED audience only. Returns null
// for non-parent events (no regex text-scan).
function formatParentAudienceLine(event, icon = AUDIENCE_LINE_ICON) {
  if (event?.audience !== "הורים") return null;
  const grade = detectGradeRange(event?.name || "", event?.description || "");
  if (grade) return `${icon} הורי ${grade}`;
  return `${icon} להורים`;
}
...
function detectBabyStage(name = "", description = "") {
  ...
  if (!text) return null;
  // Parent-focused events declare it in the TITLE ("הרצאה להורים", "מעגל הורים").
  // Test the NAME only — a toddler activity that merely mentions a parent in the
  // DESCRIPTION ("בליווי ההורים") must still keep its child age-band
  // ("זחילה-שלוש"), not fall back to a numeric "6 חודשים עד שלוש".
  if (PARENT_CONTEXT_SKIP_BABY_STAGES.test(name || "")) return null;
...
function formatBabyAudiencePrimary(event, dispMin, dispMax) {
  // Prefer the typed age_range (Gemini's lossless wording, e.g. "זחילה עד שלוש")
  // when present — no render-time regex re-derivation. Old events (no age_range)
  // fall through to the legacy title/description heuristics below.
  const typed = formatAgeRangeLabel(event?.age_range);
...
function formatAudienceLine(event) {
  ...
  if (!event.audience) {
    const name = event.name || "";
    const desc = event.description || "";
    // Un-enriched: no structured audience to trust. Show only what's derivable
    // from the structured age bounds — no regex audience/parent guessing.
    ...
    const gradeLabel = formatGradeAudienceLabel(detectGradeRange(name, desc), false);
  ...
  const babyLine = formatBabyAudienceLine(event, dispMin, dispMax, icon);
  if (babyLine) return babyLine;
  // Without a meaningful FLOOR (min null/0), rendering the range as "לידה עד X"
  // wrongly implies "from birth" for a ילדים/נוער/הורים event. Show the audience
  // label instead (e.g. ילדים). A real positive floor still renders a range.
  const label = AUDIENCE_FALLBACK_LABEL[event.audience];
  if ((dispMin == null || dispMin === 0) && label) return `${icon} ${label}`;
  const range = formatAgeRange(dispMin, dispMax);
  if (range) return `${icon} ${range}`;
...
const { formatAgeRangeLabel } = require("./eventAge");
