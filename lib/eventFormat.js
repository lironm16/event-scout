...
function effectiveEventAgeBounds(event) {
  ...
  const inferred = inferAgeBoundsFromName(event?.name || "", event?.description || "");
  return {
...
function formatAgeRange(min, max) {
  ...
  if (min != null) {
    ...
    // Adult door policy — "18+" reads clearer than "שמונה עשרה+".
    if (max == null && min >= 216 && min % 12 === 0) return `${min / 12}+`;
...
function formatAudienceLine(event) {
  ...
  if (event.audience === "מבוגרים") {
    ...
    const adultMin = min ?? inferred?.min_months ?? null;
    const adultMax = max ?? inferred?.max_months ?? null;
    const range = formatAgeRange(adultMin, adultMax);
    if (range) return `${icon} ${range}`;
