...
function formatAdultAgeGate(event) {
  if (!event || event.audience !== "מבוגרים") return null;
  ...
  // Only fire for adult-tier baselines and above. Anything below 216
  // months (18y) on a 'מבוגרים'-tagged row is a data-quality issue,
  // not an age gate — silently skip rather than emit "לגילאי 1+".
  if (min != null && min < 216) return null;
  const toYears = (m) => Math.round(m / 12);
  if (min != null && max != null) {
    return `🎯 לגילאי ${toYears(min)}-${toYears(max)}`;
  }
  if (min != null) {
    return `🎯 לגילאי ${toYears(min)}+`;
  }
  return `🎯 לגילאי עד ${toYears(max)}`;
}
