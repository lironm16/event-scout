...
const { audienceVerdict, deriveDefaultAudienceSet, deriveExcludedSubtypeTags, ageMatches } = require("./categories");
...
function passesProfileFilters(event, profile, defaultAudienceSet) {
  ...
  // Adult SUBTYPE narrowing (May-2026). Inside the `מבוגרים` audience
  // tier, exclude rows tagged with the opposite subtype for the
  // user's `age_range`: young_adult drops `גיל הזהב` tagged events,
  // senior drops `צעירים` tagged events. Same logic as search_events
  // so newsletter digests and live search stay aligned.
  if (event.audience === "מבוגרים") {
    const excludedTags = deriveExcludedSubtypeTags(profile);
    if (excludedTags.length) {
      const tags = Array.isArray(event.tags) ? event.tags : [];
      if (tags.some((t) => excludedTags.includes(t))) return false;
    }
  }

  // Age filter — only when the user has kids in the profile. ageMatches
  // is permissive on null/missing event ranges (returns true), so this
  // doesn't filter out generic/all-ages events.
  const kids = profile?.user_context?.kids || [];
  if (Array.isArray(kids) && kids.length) {
    const ages = kids.map((k) => k.age).filter(Number.isFinite);
    if (ages.length) {
      const eventLabels = {
        min_months: event.min_months,
        max_months: event.max_months,
      };
      if (!ageMatches(eventLabels, ages)) return false;
    }
  }
  return true;
}
...
function eventQualifiesForUser(event, cacheEntry) {
  ...
  // Hard: access scope. The event's `access` ENUM must be in the
  // user's allowed scope set ('open' + member communities).
  if (event.access && !accessScopes.includes(event.access)) return false;
...
async function hydrateEventsById(ids) {
  ...
  const all = await getAllEvents({
    ...
    accessScopes: ["open", "community-disabilities", "community-lgbtq", "community-seniors", "community-miluim", "community-russian"],
  });
