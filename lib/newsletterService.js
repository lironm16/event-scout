...
const { LOW_STOCK_THRESHOLD } = require("./eventCard");
const { getMutedKeys, isEventMuted } = require("./muteService");
...
function computeScore(event, interestsLower, dislikedTagsLower) {
  ...
  // Low-stock nudge — if the event is in the ⚠️ ≤9 zone, surface
  ...
  if (
    event.tickets_left != null &&
    event.tickets_left > 0 &&
    event.tickets_left <= LOW_STOCK_THRESHOLD
  ) {
    score += 5;
  }
  return score;
 ...
 * @param {{ forPreview?: boolean }} [opts]
 *   forPreview — skip the "new since last_sent_at" gate so /newsletter_preview
 *   always shows sample cards on demand (does not consume the real digest queue).
 * @returns {Promise<{ events: Array, lastSentAt: string|null, reason: 'no_profile'|'empty'|'ok' }>}
 */
async function generateUserNewsletter(telegramId, { forPreview = false } = {}) {
  ...
  // Hard: "new since last delivery" — skipped for user-initiated preview
  // so tapping preview always shows matching cards, not only not-yet-mailed rows.
  if (!forPreview) {
  ...
  // 🔕 Drop events the user muted for notifications (mute ≠ suppress — they're
  // still visible in the Mini App catalog, just not pushed here).
  const mutedKeys = await getMutedKeys(id).catch(() => new Set());
  if (mutedKeys.size) {
    events = events.filter((e) => !isEventMuted(e, mutedKeys));
  }
  ...
  // Hard: profile + audience + age + suppressed tags / online opt-out.
...
async function hydrateEventsById(ids) {
  ...
  const all = await getAllEvents({
    ...
    accessScopes: ["open", "community-disabilities", "community-lgbtq", "community-miluim", "community-russian", "community-olim"],
  });
