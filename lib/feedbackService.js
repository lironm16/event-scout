...
const REASON_LABELS = {
  wrong_audience: "👤 לא הקהל יעד שלי",
  too_far: "📍 רחוק / לא נוח להגיע",
  wrong_time: "🕒 לא בזמן שמתאים לי",
  not_interested: "🎯 הנושא לא מעניין אותי",
  already_seen: "👁️ כבר ראיתי",
  already_known: "🔁 זה אירוע חוזר — אין צורך להציג לי",
  other: "✏️ אחר…",
};
...
const ACK_LABELS = {
  wrong_audience: "✅ לא אציג עוד אירועים לקהל הזה",
  too_far: "✅ לא אציג שוב אירועים מהמקום הזה",
  wrong_time: "✅ תודה — נלמד את העדפות הזמן שלך",
  not_interested: "✅ אציג פחות מאירועים כאלה",
  already_seen: "✅ תודה",
  already_known: "✅ לא אציג שוב את הסדרה הזו",
  other: "✅ תודה",
};
...
/** All event IDs this user marked "לא מתאים" (any reason). */
async function getUserFeedbackEventIds(telegramId) {
  ...
  if (error) {
    ...
    console.warn(`[Feedback] getUserFeedbackEventIds failed: ${error.message}`);
    return new Set();
  }
