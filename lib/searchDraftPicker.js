// Multi-select search hub — toggle filters on one keyboard, then «חפשי».
...
function draftToFilters(draft) {
  ...
  if (!filters.date_preset) filters.date_preset = "this_week";
  return filters;
}
...
function buildSearchDraftKeyboard(draft) {
  const d = draft || emptyDraft();
  const rows = [];
  ...
  rows.push([
    Markup.button.callback("🔍 חפשי", "rtr:go"),
    Markup.button.callback("🗑️ נקה", "rtr:clear"),
  ]);
...
function buildSearchDraftHeader(draft, draftText) {
  const d = draft || emptyDraft();
  const lines = [
    "🔍 *חיפוש אירועים*",
    "",
    "סמני כמה מסננים שרלוונטיים (✅), ואז «חפשי». אפשר לשלב תאריך + נושאים + קהל.",
  ];
...
async function openSearchDraftHub(ctx, sessionStore, { draftText = null } = {}) {
  ...
  const text = buildSearchDraftHeader(draft, draftText);
  const keyboard = buildSearchDraftKeyboard(draft);
...
async function editSearchDraftHub(ctx, sessionStore) {
  ...
  const text = buildSearchDraftHeader(state.draft, state.draftText);
  const keyboard = buildSearchDraftKeyboard(state.draft);
  try {
