...
function buildMainMenuKeyboard({ agentEnabled, hasDraft }) {
  ...
  // Profile button opens the Mini App directly (web_app) when configured;
  // falls back to the in-bot callback otherwise.
  const profileRow = profileUrl
    ? [Markup.button.webApp("📋 פרופיל אישי", profileUrl)]
  ...
  const rows = [
    ...
    [
      // Catalog opens the Mini App directly (web_app) when configured.
      catalogUrl
