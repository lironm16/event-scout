...
async function finalizeKidAndAskMore(ctx, session, eventId, { hideEvent, appendKidToProfile }) {
  ...
  try {
    ...
    const gender = normalizeKidGender(w.draft.gender);
    if (gender) entry.gender = gender;
    await appendKidToProfile(ctx.from.id, entry);
...
function registerAudienceWizardHandlers(bot, deps) {
  ...
  bot.action(/^fb:aud:03:(\d+)$/, async (ctx) => {
    ...
    await replyAsCallbackResult(ctx, birthDatePromptForKid(null), {
      parse_mode: "Markdown",
  ...
  bot.action(/^fb:stgn:(\d+)$/, async (ctx) => {
    ...
    await ctx.answerCbQuery().catch(() => {});
    w.step = "gender";
    await ctx.reply(`⚧ מגדר ${kidNounDefiniteHe(w.draft?.gender)} — בן, בת, או לא מציין:`, {
      ...Markup.inlineKeyboard(buildKidGenderKeyboard("fb", eventId)),
    });
  });

  bot.action(/^fb:kgen:(\d+):(male|female|skip)$/, async (ctx) => {
    const [, eventId, choice] = ctx.match;
    const session = sessionStore.ensureSession(ctx.from.id);
    const w = wizardState(session);
    if (!w || String(w.eventId) !== eventId || w.step !== "gender") return;
    await ctx.answerCbQuery().catch(() => {});
    w.draft.gender = choice === "skip" ? null : normalizeKidGender(choice);
    w.step = "name_text";
    await ctx.reply(kidNameStepBody(w.draft.gender), { parse_mode: "Markdown" });
  });
  ...
  bot.action(/^fb:aud:cm:(\d+)$/, async (ctx) => {
    ...
    await replyAsCallbackResult(
      ctx,
      `${ask} את כל הרלוונטיות (עדכון כללי לפרופיל):`,
  ...
  bot.action(/^fb:cmd:(\d+)$/, async (ctx) => {
    ...
    const excludeKeys = st.excludeKeys || [];
    delete session.pendingFeedbackCommunities;
    try {
      for (const key of excludeKeys) {
        if (key) await setCommunityNotMember(ctx.from.id, key);
      }
      const profile = await getProfile(ctx.from.id);
      const ctxData = profile?.user_context || {};
      const communities = communitiesFromPickerSelection(
        st.member,
        ctxData.communities,
      );
      await supabase
      ...
      await hideEvent(ctx.from.id, eventId, "wrong_audience");
      const ack =
        excludeKeys.length > 1
          ? "✅ עדכנתי קהילות — האירוע הוסתר"
          : excludeKeys.length === 1
            ? `✅ עדכנתי — האירוע הוסתר`
            : "✅ עדכנתי קהילות — האירוע הוסתר";
