...
async function logClick(telegramId, ticketId) {
  const { error } = await supabase.from("click_log").insert({
    telegram_id: telegramId,
    ticket_id: ticketId,
  });
  if (error) console.error("[TicketService] Click log error:", error.message);
}
