...
const LEVELS = [
  ...
  { id: "before",      label: "לפני התחלת תהליך" },
...
function sanitizeKidDevStages(raw) {
  ...
  for (const [k, v] of Object.entries(raw)) {
    if (STAGE_IDS.has(k) && LEVEL_IDS.has(v) && v !== "na") out[k] = v;
  }
  return out;
}
...
function effectiveKidLevel(kid, stageId) {
  const explicit = kid?.dev_stages?.[stageId];
 ...
 * stage equals the target level. "before" targets also accept a kid "during"
 * (a prep event is still useful once just-started); never matches "na".
 */
function kidMatchesDevTargets(devTargets, kidsProfiles) {
  ...
  for (const kid of kidsProfiles) {
    for (const t of targets) {
      ...
      if (t.level === "before" && lvl === "during") return true; // prep still fits the just-started
    }
  }
  return false;
}
