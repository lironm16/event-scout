...
const PLACEHOLDER_KEYS = new Set([
  ...
  "?", "??", "???", "-", "--", "---", "—",
    // City-wide / multi-venue placeholders — municipality CMS uses these
  // when an event spans several locations (e.g. שיעורי שבועות ברחבי העיר).
  // Geocoding "רחבי העיר, רמת גן" returns city-hall, which is wrong.
  // These are stored as placeholder rows (kind='placeholder') but
  // displayLocationText recognises them and surfaces "ברחבי העיר".
  "רחבי העיר", "ברחבי העיר", "כלל העיר", "ברחבי העיר ורמת גן",
  "מספר מיקומים", "מיקומים שונים", "אתרים שונים",
]);
 ...
 * @returns {string|null}
 */
// City-wide placeholder keys — these show a friendly label instead of null.
const CITY_WIDE_KEYS = new Set([
  "רחבי העיר", "ברחבי העיר", "כלל העיר", "ברחבי העיר ורמת גן",
  "מספר מיקומים", "מיקומים שונים", "אתרים שונים",
]);
...
function displayLocationText(loc) {
  ...
  if (loc.kind === "placeholder") {
    // City-wide events: show "ברחבי העיר" instead of nothing.
    const k = normalizeKey(loc.raw_address);
    if (k && CITY_WIDE_KEYS.has(k)) return "ברחבי העיר";
    return null;
  }
...
/**
 * Returns true when the location_key (or raw address string) refers to
 * a city-wide / multi-venue placeholder — i.e. "ברחבי העיר" and friends.
 * Unlike isPlaceholderAddress (which returns true for ALL placeholders
 * including plain "אין מיקום"), this is only true when the placeholder
 * is a known city-wide indicator. Use this to render a "scattered across
 * the city" badge instead of a navigable 📍 pin.
 */
function isCityWideLocation(text) {
  const k = normalizeKey(text);
  return k ? CITY_WIDE_KEYS.has(k) : false;
}
