...
// ─────────────────────────────────────────────────────────────────────
const AUDIENCES = ["תינוקות", "ילדים", "נוער", "הורים", "מבוגרים", "ותיקים", "נשים", "לכל המשפחה"];
const CATEGORIES = [
  "סדנה",
  "הצגה",
  "הופעה",
...
- audience: ערך יחיד מהאניום הסגור: תינוקות | ילדים | נוער | הורים | מבוגרים | ותיקים | נשים | לכל המשפחה
...
- access: מערך scopes מהאניום הסגור (open | community-lgbtq | community-russian | community-seniors | community-miluim | community-olim | community-disabilities). ברירת מחדל ["open"]. זה מסנן נראוּת קשיח — ראי כלל 12.
...
5. **חוק קהל יחיד** — audience מתאר את המשתתף הראשי. בחרי ערך אחד בלבד:
   ...
   - הרצאה/סדנה/מפגש שההורה (או האם) הוא המשתתף, גם אם תינוק נוכח כמלווה ("מעגל אמהות", "קבוצת הורות", "הרצאה להורים", "יועצת") → "הורים" (או "נשים" אם לנשים בלבד) — לא "תינוקות".
   ...
   - אירועים **לנשים בלבד** / "לנשים" / "סדנה לנשים" / "מרחב לנשים" / "נשים צעירות" / "אחיות" / קבוצת נשים → audience="נשים" (min_months=216 בד״כ, max_months=null). אזהרה קריטית: "נשים צעירות" / "צעירות" זה נשים מבוגרות צעירות — **לא** "נוער"! "נוער" שמור לבני 12–18 בלבד. אירוע *על* נשים (למשל "נשים בתנ״ך" כהרצאה לקהל כללי) הוא לא "נשים" — זה לפי הקהל בפועל (מבוגרים/ותיקים).
...
12. **access (זכאות/קהילה — קשיח, מסנן נראוּת)** — מערך scopes מהאניום הסגור: open | community-lgbtq | community-russian | community-seniors | community-miluim | community-olim | community-disabilities.
...
13. החזירי JSON תקין בלבד. אל תוסיפי שדות נוספים מעבר ל-6 הנדרשים.`;
...
async function applyLabels(eventId, labels, hash, { preserveAudienceCategory = true } = {}) {
  ...
  const { data: existing } = await supabase
    .from("events")
    .select(
      "audience, category, access, name, umbrella_title, description, umbrella_id, umbrellas:umbrella_id(default_category)",
    )
    .eq("id", eventId)
    .maybeSingle();
...
async function copyFromSource(srcEvent, dstEventId) {
  ...
  const { data: full, error: rdErr } = await supabase
    .from("events")
    .select("min_months, max_months, description_hash, audience, category, tag_ids, access")
    .eq("id", srcEvent.id)
    .maybeSingle();
  if (rdErr || !full) {
