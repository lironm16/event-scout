...
// "marked as processed" — both category AND audience are non-null —
// so downstream filters treat it as a permissive default AND skip it
// on every subsequent enrichment pass (the pending-events query
// selects rows where audience IS NULL or category IS NULL; setting
// both to non-null values here breaks the infinite-retry loop that
// would otherwise re-enrich the same timed-out events on every cron
// cycle, burning Gemini quota indefinitely).
function unclassifiedFallback() {
  return {
    min_months: null,
    max_months: null,
    audience: "לכל המשפחה", // must NOT be null — prevents infinite re-enrichment loop
    category: "אחר",
    tags: [],
  };
}
        ...
        if (results) {
          ...
          try {
            ...
            try {
              ...
              await applyLabels(ctx.id, labels, ctx.hash);
              classified++;
              const labelStr = `${labels.audience || "—"}/${labels.category}/${labels.min_months}-${labels.max_months}m`;
              console.log(`[Enricher] #${ctx.id} "${(ev.name || "").slice(0, 40)}" → ${labelStr} (fallback_timeout)`);
              continue;
            }
          }
        }
