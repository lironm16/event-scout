...
// ─────────────────────────────────────────────────────────────────────
const AUDIENCES = ["תינוקות", "ילדים", "נוער", "מבוגרים", "לכל המשפחה"];
const CATEGORIES = [
  "סדנה",
  "הצגה",
  "הופעה",
...
5. **חוק קהל יחיד** — audience מתאר את המשתתף הראשי. בחרי ערך אחד בלבד:
   ...
   - הופעות ערב, הרצאות מקצועיות בלי ילדים נוכחים → "מבוגרים".
...
11. החזירי JSON תקין בלבד. אל תוסיפי שדות נוספים מעבר ל-5 הנדרשים.`;
...
function extractDescription(html) {
  ...
  if (heading.length) {
    ...
    if (container.length) {
      const txt = container
        .clone()
        .find("h1, h2, h3, h4, h5, h6, script, style")
        .remove()
        .end()
        .text()
        .replace(/\s+/g, " ")
  ...
  $("script").remove();
  for (const sel of [".event_description", ".description", "#description", ".event-info", ".event_details", ".txt_container", "section.txt", "main"]) {
    const el = $(sel).first();
    if (el.length) {
  ...
  return body.slice(0, DESCRIPTION_CHAR_CAP);
}
...
function extractEventAddress(html) {
  ...
  // 3. .theater_name fallback. Strip the trailing "(מפת הגעה)" /
  //    "(directions)" / similar parenthetical that Smarticket adds
  //    as a Google-Maps link label.
  const t = $(".theater_name").first().text().replace(/\s+/g, " ").trim();
  if (t) {
...
async function enrichEventData({ id, name, source }) {
  ...
  if (source === "rg-muni") {
    ...
    const html = await fetchDetailHtml(id, name, source);
  ...
  try {
    ...
    try {
      ...
      labels = withSmarticketCluster(labels, smarticketCluster);
...
// Inject the smarticket cluster name (`breadcrumb_category`, e.g.
// "שבת משפחה קהילה") into the tag list AFTER Gemini has produced
// its semantic tags. Idempotent: if the cluster is already in the
// list (by normalized name), nothing is added.
//
...
// matters because users describe events by it ("שבת קהילה") even
// when the prose never mentions that phrase. By persisting it
// verbatim as its own tag we guarantee `search_events({ tags: [...] })`
// and the labelStore fuzzy resolver can find these events.
function withSmarticketCluster(labels, cluster) {
  if (!cluster || typeof cluster !== "string") return labels;
  ...
  if (seen.has(labelStore.normalizeName(trimmed))) return labels;
  return { ...labels, tags: [...existing, trimmed] };
}
...
async function fetchPendingEvents(limit) {
  ...
  //   2. Partial-state rows: description_hash IS NOT NULL but BOTH
  //      audience AND category are null. This pattern only happens
  //      when `applyLabels`'s first UPDATE (numeric range + hash)
  //      succeeded but `labelStore.setEventLabels`'s second UPDATE
  //      (audience/category/tag_ids) silently failed (the helper
  //      console.warns instead of throwing). Without this branch the
  //      row is frozen forever — the original `description_hash IS
  //      NULL` query skipped it on every subsequent pass.
  //
