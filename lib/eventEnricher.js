...
const { classifyFromName } = require("./eventAgeParser");
...
// Developmental-stage targeting (events.dev_stages) — a SET of discrete markers,
// matched against the kid profile's stages. Vocabulary MUST mirror kidsWizardUi
// DEV_STAGES ids. Distinct from age_range (chronological).
const DEV_STAGES = ["crawl", "walk", "wean", "solids", "talk"];
const DEV_STAGE_SET = new Set(DEV_STAGES);
function sanitizeDevStages(value) {
  if (!Array.isArray(value)) return [];
  return [...new Set(value.filter((s) => DEV_STAGE_SET.has(s)))];
}
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    // Developmental-stage targeting (SET, not a range). Empty unless the event
    // explicitly targets a stage. Vocabulary = kid-profile DEV_STAGES ids.
    dev_stages: {
      type: SchemaType.ARRAY,
      items: { type: SchemaType.STRING, format: "enum", enum: ["crawl", "walk", "wean", "solids", "talk"] },
    },
  },
  ...
  required: ["audience", "category", "tags", "min_months", "max_months", "age_range", "dev_stages"],
};
...
- dev_stages: מערך שלבי-התפתחות שהאירוע **מכוון אליהם במפורש** (לא גיל כרונולוגי אלא יכולת/שלב של הילד). אוצר-מילים סגור: crawl (זוחל) | walk (הולך) | wean (גמול/גמילה מחיתולים) | solids (אוכל מוצקים/מעבר למוצקים) | talk (מדבר/טרום-מילולי). ברירת מחדל מערך ריק []. מלאי **רק** כשהאירוע ממש מדבר על שלב — "סדנת גמילה" → ["wean"] ; "מעבר/ביסוס מוצקים" → ["solids"] ; "לתינוקות שעוד לא הולכים" / "זוחלים" → ["crawl"] ; "התפתחות שפה / טרום-מילולי" → ["talk"]. אירוע גיל רגיל בלי אזכור שלב → [].
...
14. החזירי JSON תקין בלבד. אל תוסיפי שדות מעבר ל-9 המפתחות. כל המפתחות חייבים להופיע — כולל age_range עם min ו-max (כל קצה הוא אובייקט או null; אסור להשמיט את max), ו-dev_stages (מערך, ריק אם אין שלב).`;
...
async function callGemini(title, description, labels = [], department = null) {
  ...
  return reconcileLabels({
    ...
    dev_stages: sanitizeDevStages(parsed.dev_stages),
  });
}
    ...
    reconcileLabels(
      {
        ...
        dev_stages: sanitizeDevStages(item.dev_stages),
      },
      {
        name: events[i]?.title,
...
async function applyLabels(eventId, labels, hash, { preserveAudienceCategory = true } = {}) {
  ...
  const { data: existing } = await supabase
    .from("events")
    .select(
      "audience, category, access, age_range, dev_stages, name, umbrella_title, description, umbrella_id, umbrellas:umbrella_id(default_category)",
    )
    .eq("id", eventId)
    .maybeSingle();
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      dev_stages: (hinted.dev_stages && hinted.dev_stages.length)
        ? hinted.dev_stages
        : (existing?.dev_stages ?? []),
    })
    .eq("id", eventId);
...
async function copyFromSource(srcEvent, dstEventId) {
  ...
  const { data: full, error: rdErr } = await supabase
    .from("events")
    .select("min_months, max_months, description_hash, audience, category, tag_ids, access, age_range, dev_stages")
    .eq("id", srcEvent.id)
    .maybeSingle();
  if (rdErr || !full) {
  ...
  const { error: wrErr } = await supabase
    ...
    .update({
      ...
      dev_stages: (Array.isArray(full.dev_stages) && full.dev_stages.length)
        ? full.dev_stages
        : (dst?.dev_stages ?? []),
    })
    .eq("id", dstEventId);
...
function applyRuleBasedAudienceCategory(labels, { name = "", description = "" } = {}) {
  const hay = `${name} ${description}`.replace(/\s+/g, " ");
  let audience = labels?.audience || null;
  if (!audience) {
    // Deterministic age-phrase parse FIRST (precise, rate-limit-proof) — the
    // smart fallback that replaces blind keyword-guessing. "לגילאי הליכה עד
    // שלוש" → תינוקות, not נוער. The keyword heuristics below only fire when
    // the parser isn't confident.
    const parsed = classifyFromName(name);
    if (parsed.audience) {
      audience = parsed.audience;
    } else if (/נוער|כיתות\s*ז|ט['\u05F3]?\s*י|י['\u05F3]?\s*ב/u.test(hay)) {
      // NOTE: a bare numeric age-range ("בגילאי 0-3") must NOT default to
      // נוער — that was the bug behind the נוער over-tagging. Only the literal
      // word נוער + explicit class-grades (ז'–י"ב) count as teen signals here.
      audience = "נוער";
    } else if (/תינוק|פעוט|לגיל\s*\d+\s*חודש/u.test(hay)) {
      audience = "תינוקות";
    } else if (/ילדים|לילדים|משפחה|לכל המשפחה/u.test(hay)) {
      audience = /לכל המשפחה/u.test(hay) ? "לכל המשפחה" : "ילדים";
    } else if (/הורים|להורים|הורות/u.test(hay)) {
      audience = "הורים";
    } else if (/ותיק|גיל שלישי|מבוגר/u.test(hay)) {
      audience = /ותיק|גיל שלישי/u.test(hay) ? "ותיקים" : "מבוגרים";
    }
  }
  let category = labels?.category || null;
  if (!category) {
    category =
  ...
  return { ...labels, audience, category };
}
