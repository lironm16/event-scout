...
- age_range: ייצוג הטווח ב**מילים המקוריות** (לתצוגה), **בנוסף** ל-min/max_months (שנשארים לפי כללי החודשים למטה). אובייקט {min, max}; כל קצה {kind:"stage"|"months"|"years", value (כמחרוזת), inclusive}. stage ∈ birth|crawl|walk (לידה/זחילה/הליכה). value מספרי ביחידה המקורית (לא להמיר). inclusive=false ל"לא כולל". קצה null = פתוח; כל השדה null אם אין מידע גיל. דוגמאות: "זחילה עד שלוש" → {min:{kind:"stage",value:"crawl",inclusive:true}, max:{kind:"years",value:"3",inclusive:true}} ; "לגילאי 3-6" → {min:{kind:"years",value:"3",inclusive:true}, max:{kind:"years",value:"6",inclusive:true}} ; "3-6 חודשים" → kind:"months" ; "עד זחילה לא כולל" → {min:null, max:{kind:"stage",value:"crawl",inclusive:false}}.
...
async function applyLabels(eventId, labels, hash, { preserveAudienceCategory = true } = {}) {
  ...
  const { data: existing } = await supabase
    .from("events")
    .select(
      "audience, category, access, age_range, name, umbrella_title, description, umbrella_id, umbrellas:umbrella_id(default_category)",
    )
    .eq("id", eventId)
    .maybeSingle();
...
async function copyFromSource(srcEvent, dstEventId) {
  ...
  const { data: full, error: rdErr } = await supabase
    .from("events")
    .select("min_months, max_months, description_hash, audience, category, tag_ids, access, age_range")
    .eq("id", srcEvent.id)
    .maybeSingle();
  if (rdErr || !full) {
