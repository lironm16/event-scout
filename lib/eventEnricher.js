...
// Series-parent normalization (sql/086): route a freshly-fetched
// description to the series umbrella (stored once) instead of
// re-duplicating it onto an already-linked recurring child.
const { persistChildDescription } = require("./seriesParentService");
...
const DESCRIPTION_CHAR_CAP = 6000; // raised from 2000 — long programme blurbs were truncated
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    // addresses. Empty unless the event explicitly targets a stage. Both a prep
    // event AND an event for kids mid-process → level "during" (the "before"
    // level was retired — anyone IN the process should get the stage's events);
    // an event for kids who already MASTERED X → "established".
    dev_targets: {
      type: SchemaType.ARRAY,
      items: {
        type: SchemaType.OBJECT,
        properties: {
          ...
          level: { type: SchemaType.STRING, format: "enum", enum: ["during", "established"] },
        },
...
- access: מערך scopes מהאניום הסגור (open | community-lgbtq | community-russian | community-miluim | community-olim | community-disabilities | community-women). ברירת מחדל ["open"]. זה מסנן נראוּת קשיח — ראי כלל 12.
...
- dev_targets: מערך של {stage, level} — לאיזה שלב התפתחותי **ובאיזו רמת מוכנות** האירוע מכוון. stage ∈ solids (מוצקים) | crawl (זחילה) | walk (הליכה) | talk (דיבור) | wean (גמילה מחיתולים). level ∈ during (בתהליך — כולל גם הכנה/מעבר וגם תוך-כדי; כל מי שבדרך לשלב או בתוכו) | established (כבר מבוסס/סיים). **אין יותר "before"** — אירוע הכנה/מעבר וגם אירוע תוך-כדי שניהם **during** (הרעיון: מי שבתהליך מקבל את אירועי השלב). ברירת מחדל [] (רוב האירועים). מלאי **רק** כשהאירוע ממש מדבר על שלב:
  • "סדנת מעבר/הכנה למוצקים" / "גמילה מחיתולים" → [{stage:"solids",level:"during"}] ; [{stage:"wean",level:"during"}].
  • "פעילות לזוחלים" / "לתינוקות שכבר זוחלים" / "לקראת זחילה" → [{stage:"crawl",level:"during"}].
  • "התפתחות שפה / טרום-מילולי" → [{stage:"talk",level:"during"}].
...
5. **חוק קהל יחיד** — audience מתאר את המשתתף הראשי. בחרי ערך אחד בלבד:
   ...
   - **חריג קריטי — סדנת התפתחות/העשרה לתינוק/לפעוט:** פעילות שתוכנה פיתוח/גירוי/העשרה של הילד (תקשורת, שפה, מוזיקה, תנועה, סנסורי, עיסוי תינוקות, "התפתחות", "גן יחד") — **גם אם ההורה מבצע את הטכניקות והתינוק "רק נוכח"** — מסווגת **לפי גיל הילד** ("תינוקות"/"ילדים"), לא "הורים". התוכן הוא לטובת הילד. דוגמה: "בה בה בה סדנת תקשורת ושפה מלידה-4 חודשים" → **"תינוקות"**.
   - "הורים" שמור אך ורק לתוכן שמכוון **להורה עצמו** ולא לפיתוח הילד: "מעגל אמהות", "קבוצת הורות"/קבוצת תמיכה, "הרצאה להורים", ייעוץ שינה/האכלה/הנקה, הריון, "יועצת" — גם אם תינוק נוכח כמלווה.
   ...
   - **גיל מינימום פתוח ("14+", "מגיל 13 ומעלה", "13+" — בלי תקרה עליונה) אינו "נוער"!** "נוער" שמור רק לאירוע שמכוון ספציפית לבני 12–18 (עם תקרה ~18). אירוע "X+" פתוח (גם אם X בטווח הנעורים) מיועד לכל מי שמעל X — נוער, צעירים, מבוגרים **וגם** ותיקים. במקרה כזה: audience="מבוגרים", min_months=X×12, max_months=null. ההתאמה לקהלים השונים נעשית לפי **טווח‑הגיל** (min/max_months), לא לפי ערך ה‑audience היחיד.
...
12. **access (זכאות/קהילה — קשיח, מסנן נראוּת)** — מערך scopes מהאניום הסגור: open | community-lgbtq | community-russian | community-miluim | community-olim | community-disabilities | community-women.
...
async function copyFromSource(srcEvent, dstEventId) {
  ...
  const { data: dst } = await supabase
    .from("events")
    .select(
      "audience, category, access, age_range, dev_targets, emoji, name, umbrella_title, description, umbrella_id, umbrellas:umbrella_id(default_category, series_key, description)",
    )
    .eq("id", dstEventId)
    .maybeSingle();

  // Series child (sql/086)? Then the prose lives ONCE on the parent
  // umbrella — never copy it back onto the child, or the cache hit would
  // re-duplicate exactly what the normalization removed. Route the source's
  // description up to the parent when it has none / a shorter one instead.
  const dstSeriesParent =
    dst?.umbrella_id && dst?.umbrellas?.series_key != null ? dst.umbrellas : null;
  if (dstSeriesParent) {
    const srcDesc = (full.description || "").trim();
    const parentDesc = (dstSeriesParent.description || "").trim();
    if (srcDesc && srcDesc.length > parentDesc.length) {
      await supabase
        .from("umbrellas")
        .update({ description: srcDesc, updated_at: new Date().toISOString() })
        .eq("id", dst.umbrella_id);
    }
  }
  ...
  const { error: wrErr } = await supabase
    ...
    .update({
      ...
      // description the destination already has. EXCEPTION: a series child
      // (sql/086) keeps NULL — the prose was routed to the parent above and
      // the read path resolves it via umbrella_parent.description.
      description: dstSeriesParent ? null : (dst?.description || full.description || null),
...
async function enrichEventData({ id, name, source }) {
  ...
  if (source === "rg-muni") {
    ...
    // Persist the city prose so the Mini App shows it even when Gemini can't
    // be reached (quota). persistChildDescription writes the child only when
    // its column is null AND it isn't a series child — for a series child the
    // prose is routed to the parent umbrella (sql/086) so it's stored once.
    if (description) {
      await persistChildDescription(id, description);
    }
    ...
    // Persist description so the Mini App can display it. For a series child
    // the prose is routed to the parent umbrella (stored once, sql/086);
    // otherwise written to the child only when its column is null.
    if (description) {
      await persistChildDescription(id, description);
    }
...
async function enrichPendingEvents(limit = MAX_PER_CYCLE) {
  ...
  // Reconcile recurring same-name series into series-parent umbrellas
  // (sql/086) so a description fetched this cycle for a new occurrence is
  // de-duplicated up to the parent. Batch + best-effort — never block or
  // fail the enrich cycle on it.
  //
  // GATED behind SERIES_PARENT_RECONCILE=1: reconcile NULLs child
  // descriptions, which only render correctly where the read-path fallback
  // (this commit) is deployed. dev + prod share one DB, so we keep this OFF
  // until BOTH environments run the fallback code — then flip the flag.
  if (process.env.SERIES_PARENT_RECONCILE === "1") try {
    const { reconcileSeriesParents } = require("./seriesParentService");
    const rep = await reconcileSeriesParents({ apply: true });
    if (rep.series) {
      console.log(`[Enricher] series-parent reconcile: ${rep.series} series, ${rep.children} children, ~${Math.round(rep.freedBytes / 1024)}KB freed`);
    }
  } catch (err) {
    console.warn(`[Enricher] series-parent reconcile failed: ${err.message}`);
  }
