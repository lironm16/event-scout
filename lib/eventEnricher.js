...
async function applyLabels(eventId, labels, hash, { preserveAudienceCategory = true } = {}) {
  ...
  const { data: existing } = await supabase
    .from("events")
    .select(
      "audience, category, access, age_range, dev_stages, emoji, name, umbrella_title, description, umbrella_id, umbrellas:umbrella_id(default_category)",
    )
    .eq("id", eventId)
    .maybeSingle();
...
async function copyFromSource(srcEvent, dstEventId) {
  ...
  const { data: full, error: rdErr } = await supabase
    .from("events")
    .select("min_months, max_months, description_hash, audience, category, tag_ids, access, age_range, dev_stages, emoji")
    .eq("id", srcEvent.id)
    .maybeSingle();
  if (rdErr || !full) {
