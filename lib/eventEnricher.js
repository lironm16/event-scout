...
async function applyLabels(eventId, labels, hash) {
  ...
  //
  // We ALSO read `name`, `umbrella_title`, and `description` so the
  // category fallback chain can fire `inferCategoryFromName` /
  // `inferCategoryFromUmbrellaTitle` / `inferCategoryFromDescription`
  // when Gemini returned null AND no prior value exists — see the
  // chain assembly below.
  const { data: existing } = await supabase
    .from("events")
    .select("audience, category, name, umbrella_title, description")
    .eq("id", eventId)
    .maybeSingle();
  ...
  //          empty.)
  //   6. Umbrella title scan                             — PARENT, last
  //         (`inferCategoryFromUmbrellaTitle` — any-word
  //          match. Only runs if the child gave us
  //          absolutely nothing.)
  //
...
async function copyFromSource(srcEvent, dstEventId) {
  ...
  //   name strict → description → name loose → umbrella
