...
// Concepts already captured by a dedicated structured field — an `audience`
// value or an `access` community — must NOT also come back as a free `tag`
// (redundant, and can conflict with the structured field). e.g. "נוער" is the
// audience, "קהילה גאה" is the access community — neither should be a tag.
// (Event CATEGORIES like "ספורט"/"מסיבה" are intentionally NOT included — they
// double as useful topic tags.) Matched after a light normalize.
const { AUDIENCE_CATEGORIES: _AUD_CATS } = require("./interestCategories");
function _normTagConcept(s) {
  return String(s || "").replace(/["'״׳]/g, "").replace(/\s+/g, " ").trim();
}
const COVERED_TAG_CONCEPTS = new Set(
  [
    ...AUDIENCES,
    ..._AUD_CATS.filter((a) => a.community).map((a) => a.label),
    // common variants Gemini tends to emit for the above:
    "הקהילה הגאה", "להטב", 'להט"ב', "גאווה", "נשים בלבד", "גברים בלבד",
    "מילואים", "עולים", "דוברי רוסית", "ותיקים", "גיל הזהב", "גיל הזהב 60+",
  ].map(_normTagConcept),
);
function isCoveredTagConcept(tag) {
  return COVERED_TAG_CONCEPTS.has(_normTagConcept(tag));
}
...
14. החזירי JSON תקין בלבד. אל תוסיפי שדות נוספים מעבר ל-7 הנדרשים.`;
