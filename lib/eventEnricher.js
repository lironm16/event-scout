...
- audience: ערך יחיד מהאניום הסגור: תינוקות | ילדים | נוער | הורים | מבוגרים | ותיקים | לכל המשפחה
...
// schema make the model stall indefinitely). Returns a minimal object
// with NO audience or category so no automatic classification is written.
// The caller is responsible for stamping enrichment_failed_at on the DB
// row (via markEnrichmentFailed) to prevent the cron from retrying.
...
/**
 * Stamp enrichment_failed_at on an event row so the cron skips it.
 * audience and category stay NULL — no automatic classification is written.
 * To retry: UPDATE events SET enrichment_failed_at = NULL WHERE id = X;
 */
...
async function enrichEventData({ id, name, source }) {
  ...
  // 4. Cache miss → Gemini. On timeout we give up immediately (no
  //    retry): write null audience/category, stamp enrichment_failed_at,
  //    and leave manual review to the operator.
  let labels;
  try {
    labels = await callGemini(name, description, siteLabels, department);
  } catch (err) {
    if (!isTimeoutError(err)) throw err;
    console.warn(`[Enricher] #${id} Gemini timed out — writing unclassified fallback (audience=null, needs review).`);
    labels = unclassifiedFallback();
    labels = withPreservedClusters(
      labels,
      source === "rg-muni" ? siteLabels : smarticketCluster,
    );
    await applyLabels(id, labels, hash, { preserveAudienceCategory: false });
    await markEnrichmentFailed(id);
    return { source: "fallback_timeout", labels, needsReview: true };
  }
  ...
  labels = withNameBasedTags(labels, name);
...
// ─────────────────────────────────────────────────────────────────────
// Name-based tag injection — applied AFTER Gemini so these tags are
// always present regardless of what the model infers from the prose.
// The enricher prompt explicitly instructs Gemini NOT to echo
// department/category names from the breadcrumb, so some obvious
// semantic tags (e.g. "משחקיות" for playroom events) are reliably
// missing from the LLM output and must be injected here.
//
// Tag names must match an existing `labels` row exactly (case/diacritics
// normalised by labelStore) — unresolvable names are silently dropped.
// ─────────────────────────────────────────────────────────────────────
const ENRICHER_NAME_TAG_RULES = [
  { re: /משחקיה|משחקייה/u,  tags: ["משחקיות"] },
];

function withNameBasedTags(labels, name) {
  if (!name || !ENRICHER_NAME_TAG_RULES.length) return labels;
  const existing = Array.isArray(labels?.tags) ? labels.tags : [];
  const seen = new Set(existing.map((t) => labelStore.normalizeName(String(t))));
  const merged = [...existing];
  for (const { re, tags } of ENRICHER_NAME_TAG_RULES) {
    if (!re.test(name)) continue;
    for (const t of tags) {
      const norm = labelStore.normalizeName(t);
      if (!seen.has(norm)) { seen.add(norm); merged.push(t); }
    }
  }
...
async function fetchPendingEvents(limit) {
  ...
  //
  // Selection criteria — pick up rows that need enrichment:
  //   1. Never-enriched: description_hash IS NULL
  //   2. Partial-state: description_hash IS NOT NULL but BOTH audience
  //      AND category are null. This catches a labelStore write crash
  //      that left nothing written after the hash was stamped.
  //
  // Explicitly excluded:
  //   - enrichment_failed_at IS NOT NULL — Gemini timed out;
  //     audience/category left null for manual review. Reset to NULL
  //     to allow a retry: UPDATE events SET enrichment_failed_at=NULL.
  //   - archived rows.
  //
  // We do NOT sweep rows with null audience alone — that would cause
  // an infinite re-enrichment loop for fallback events (May-2026 bug).
  const { data, error } = await supabase
    .from("events")
...
async function _applyGeminiResult(ctx, labels) {
  ...
  labels = withNameBasedTags(labels, ctx.name);
  ...
  for (let i = 0; i < events.length; i++) {
    ...
    try {
      ...
      if (prep.cached) {
        ...
        console.log(
          `[Enricher] #${ev.id} "${(ev.name || "").slice(0, 40)}" ← #${prep.result.source_event_id} (${prep.result.source})`,
        );
      } else {
      ...
      try {
        ...
        if (results) {
          ...
          try {
            ...
            if (!isTimeoutError(err)) throw err;
            console.warn(`[Enricher] #${ctx.id} Gemini timed out — writing unclassified fallback.`);
            labels = unclassifiedFallback();
            labels = withPreservedClusters(labels, ctx.preservedClusters);
            await applyLabels(ctx.id, labels, ctx.hash, { preserveAudienceCategory: false });
            await markEnrichmentFailed(ctx.id);
            classified++;
            fallbackEvents.push({ id: ctx.id, name: ev.name });
            console.log(`[Enricher] #${ctx.id} "${(ev.name || "").slice(0, 40)}" → fallback_timeout (audience=null, needs review)`);
            continue;
          }
        }
