...
function extractDescription(html) {
  ...
  if (heading.length) {
    const container = heading.closest(".txt_container").length
      ...
      : heading.parent();
    if (container.length) {
      const txt = cleanDescription(container
        .clone()
    ...
    try {
      ...
      for (const item of items) {
        if (item && item["@type"] === "Event" && typeof item.description === "string") {
          const txt = cleanDescription(item.description.replace(/\s+/g, " ").trim());
          if (txt.length > 40) return txt.slice(0, DESCRIPTION_CHAR_CAP);
        }
      }
    } catch {
  ...
  for (const sel of [".event_description", ".description", "#description", ".event-info", ".event_details", ".txt_container", "section.txt", "main"]) {
    const el = $(sel).first();
    if (el.length) {
      const txt = cleanDescription(el.text().replace(/\s+/g, " ").trim());
      if (txt.length > 40) return txt.slice(0, DESCRIPTION_CHAR_CAP);
    }
  }

  const body = cleanDescription($("body").text().replace(/\s+/g, " ").trim());
