...
// "marked as processed" via category="אחר" (non-null) but leave
// audience=null intentionally — indicating "could not determine".
// The pending-events query only re-selects events where BOTH audience
// AND category are null, so category="אחר" acts as the "done" marker
// that stops the infinite-retry loop. The operator receives an alert
// so they can review/fix the audience manually.
function unclassifiedFallback() {
  return {
    min_months: null,
    max_months: null,
    audience: null, // intentionally null — "unknown, needs manual review"
    category: "אחר",
    tags: [],
  };
}
...
async function enrichEventData({ id, name, source }) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`[Enricher] #${id} Gemini timed out twice — writing unclassified fallback (audience=null, needs review).`);
...
async function fetchPendingEvents(limit) {
  ...
  //   2. Truly partial-state rows: description_hash IS NOT NULL AND
  //      BOTH audience AND category are null. This catches the case
  //      where labelStore.setEventLabels crashed mid-way and left
  //      nothing written. Requiring BOTH null (not just one) is
  //      critical: unclassifiedFallback writes category="אחר" while
  //      leaving audience=null intentionally (operator review). If we
  //      selected on audience IS NULL alone, those timed-out events
  //      would be re-enriched on every cron cycle — up to 16,800
  //      Gemini calls/day — an infinite cost loop (May-2026 regression).
  //      category="אחר" acts as the "processed" sentinel that stops
  //      re-selection; audience=null remains visible for manual review.
  //
  // We deliberately don't sweep up rows with empty tag_ids or null
  // age range — those are valid Gemini outputs (e.g. an open-bound
  // adult lecture), and re-enriching them would burn quota for no gain.
  const { data, error } = await supabase
    .from("events")
    ...
    for (let j = 0; j < chunk.length; j++) {
      ...
      try {
        ...
        if (results) {
          ...
          // Individual fallback with one retry on timeout
          try {
            ...
            try {
              ...
              if (!isTimeoutError(err2)) throw err2;
              console.warn(`[Enricher] #${ctx.id} Gemini timed out twice — writing unclassified fallback.`);
              labels = unclassifiedFallback();
              labels = withPreservedClusters(labels, ctx.preservedClusters);
              await applyLabels(ctx.id, labels, ctx.hash);
              classified++;
