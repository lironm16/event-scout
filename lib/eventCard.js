...
//   1..10 tickets   → "🎫 N כרטיסים אחרונים ❗️"
...
function formatTicketsLine(ticketsLeft) {
...
function buildNavigateButton(event) {
  ...
  if (coords && Number.isFinite(Number(coords.lat)) && Number.isFinite(Number(coords.lng))) {
    // Fix to 6 decimal places — that's ~11cm of precision, more
    // than enough for a venue pin, and keeps the callback_data
    // length deterministic (max ~28 bytes for lat,lng with both
    // sign-prefixed). Avoids "callback data too long" failures
    // for events with high-precision coords.
    const lat = Number(coords.lat).toFixed(6);
    const lng = Number(coords.lng).toFixed(6);
    return { text: "🧭 ניווט", callback_data: `nav:${lat},${lng}` };
  }
  // No coords → fall back to a text-search URL. No picker for this
  // path: Waze/Apple deep links need real coords; Google Maps
  // search-by-text is the best universal landing.
  const venueText = pickVenueText(event);
  if (venueText) {
    const url = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(venueText)}`;
