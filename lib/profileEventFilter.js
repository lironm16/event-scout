...
/**
 * Availability window: when the profile defines availability blocks, an event
 * passes only if its START time falls inside one of them (matching the day of
 * week when blocks specify days). No availability set → always passes. Events
 * without a start time pass (we can't place them).
 */
function eventWithinAvailability(event, profile) {
  const av = profile?.user_context?.constraints?.availability;
  const blocks = Array.isArray(av?.blocks) ? av.blocks : [];
  if (!blocks.length) return true;
  const st = typeof event?.start_time === "string" ? event.start_time.slice(0, 5) : "";
  if (!st) return true;
  let dow = null;
  if (event?.date) {
    const d = new Date(`${event.date}T00:00:00`);
    if (!isNaN(d.getTime())) dow = d.getDay(); // 0=Sun … 6=Sat
  }
  return blocks.some((b) => {
    if (Array.isArray(b.days) && dow != null && !b.days.includes(dow)) return false;
    const start = (b.start || "00:00").slice(0, 5);
    const end = (b.end || "23:59").slice(0, 5);
...
// Muted recurring series ("🔁 מכירה — אירוע חוזר" → known_series). Key
// shape mirrors newsletterService.makeSeriesKey (name + location_key);
// inlined to avoid a circular require. A muted series does NOT fit the
// profile, so it must fail here too — otherwise "מתאים לפרופיל שלך" and
// the for-me lists would resurface exactly what the user muted.
function eventInKnownSeries(event, profile) {
  const list = profile?.user_context?.known_series;
  if (!Array.isArray(list) || !list.length) return false;
  const name = String(event?.name || "").trim().replace(/\s+/g, " ").toLowerCase();
  if (!name) return false;
  const key = `${name}::${event?.location_key || ""}`;
  return list.some((k) => String(k) === key);
}

/** Venue opt-out: the user marked this location as "don't show". Mirrors the
 *  search_events suppressed_locations filter so the for-me flag + notifiers
 *  agree with the catalog. */
function eventAtSuppressedLocation(event, profile) {
  const list = profile?.user_context?.preferences?.suppressed_locations;
  if (!Array.isArray(list) || !list.length) return false;
  return !!event.location_key && list.includes(event.location_key);
}
...
function passesProfileFilters(event, profile, defaultAudienceSet) {
  ...
  if (eventInKnownSeries(event, profile)) return false;
  if (eventAtSuppressedLocation(event, profile)) return false;
  if (shouldHideChildEventForProfile(event, profile)) return false;
  if (!eventWithinAvailability(event, profile)) return false;
  ...
  // (Profile-owner age-cohort filter removed — the viewer's own age no
  // longer hides adult events.)

  // Kids-age fit ONLY gates CHILD-targeted events. An adult / parent /
  // family event must not be hidden just because it doesn't fit a
  // household baby — the parent is an adult too (see "see everything"
  // default). Child events still must fit a household kid's age/stage.
  const kids = profile?.user_context?.kids || [];
  if (Array.isArray(kids) && kids.length && isChildTargetedEvent(event)) {
...
async function annotateProximity(events, profile) {
  ...
  for (const e of events) {
    ...
    const result = await evaluateProximity(
      ...
      maxWalk,
      venueCoords,
      { useRoutesApi: budget.remaining >= 35 },
    );
