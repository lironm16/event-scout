...
const { getTravelMinutesCached } = require("./travelTimeCache");
const { DEFAULT_CITY, CITY_CENTER, MAX_KM_FROM_CITY } = require("./geocodingDefaults");

// Hybrid (Option C) tuning. We only spend a Routes API call when the
// straight-line distance is NEAR a decision boundary — i.e. when the
// cheap haversine estimate might misclassify the walk/drive label or the
// "short drive" (≤10 min) pass/fail. Clearly-near and clearly-far venues
// use the heuristic for free. Borderline calls go through travelTimeCache
// (Supabase, coarse home grid) so they're shared across users + restarts.
const BORDERLINE_BAND = 0.25; // ±25% around a boundary km
const SHORT_DRIVE_MAX_MINUTES = 10; // mirrors locationPrefs.DRIVE_MAX_MINUTES
...
// Reject-far guard: a geocode result this far from the city centre is almost
// certainly a wrong-city match (same-named venue elsewhere). Returns true so
// the caller can skip it and fall through to the next geocoder / mark unfound.
function isImplausiblyFar(lat, lng) {
  if (lat == null || lng == null) return false;
  const km = haversineKm(lat, lng, CITY_CENTER.lat, CITY_CENTER.lng);
  if (km > MAX_KM_FROM_CITY) {
    console.warn(
      `[Geocoding] rejected far result (${km.toFixed(1)}km from ${DEFAULT_CITY}) at ${lat},${lng}`,
    );
    return true;
  }
  return false;
...
async function geocodeAddress(address, opts = {}) {
  ...
  try {
    ...
    if (cached) {
      let result = cached.found && cached.lat != null && cached.lng != null
        ? {
            lat: cached.lat,
            lng: cached.lng,
        ...
        : null;
      // Defensive: a value cached BEFORE the reject-far guard existed must not
      // be served — drop it (mark unfound) so the address re-resolves fresh.
      if (result && isImplausiblyFar(result.lat, result.lng)) {
        result = null;
        saveLocation({
          key, raw_address: address, source: cached.source || "db", found: false, city,
        }).catch(() => {});
      }
  ...
  if (googlePlaces.isEnabled()) {
    ...
    if (place && !isImplausiblyFar(place.lat, place.lng)) {
      const result = {
        lat: place.lat,
        lng: place.lng,
  ...
  try {
    ...
    if (isImplausiblyFar(result.lat, result.lng)) {
      // Wrong-city match — mark unfound rather than caching a bad pin.
      geocodeCache.set(key, null);
      saveLocation({
        key, raw_address: address, display_name: normalizedHint || null,
        source: "nominatim", found: false, city,
      }).catch(() => {});
      return null;
    }
    geocodeCache.set(key, result);
...
async function evaluateProximity(homeCoords, venueText, maxWalkMinutes = null, preResolvedVenueCoords = null, opts = {}) {
  ...
  // Borderline gate (Option C): only refine with the Routes API when the
  // straight-line distance sits near a boundary the heuristic could get
  // wrong. Everything clearly inside/outside keeps the free heuristic.
  const shortDriveKm = kmForDriveMinutes(SHORT_DRIVE_MAX_MINUTES);
  const nearBoundary = (boundary) =>
    boundary > 0 &&
    km >= boundary * (1 - BORDERLINE_BAND) &&
    km <= boundary * (1 + BORDERLINE_BAND);
  const borderline = nearBoundary(kmLimit) || nearBoundary(shortDriveKm);

  const apiMinutes =
    useRoutesApi && borderline
      ? await getTravelMinutesCached(homeCoords, venueCoords, mode)
      : null;
  if (apiMinutes != null) {
