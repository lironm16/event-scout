...
function registerMiniAppRoutes(app, { prefix, verifyInitData, logLabel, bot }) {
  ...
  app.post(`${prefix}/saved`, async (req, res) => {
    ...
    try {
      ...
      if (!eventId) return res.status(400).json({ error: "Missing eventId" });
      if (saved === false) {
        await supabase.from("saved_events").delete().eq("telegram_id", identity.telegramId).eq("event_id", Number(eventId));
      } else {
        await supabase.from("saved_events").upsert({ telegram_id: identity.telegramId, event_id: Number(eventId) }, { onConflict: "telegram_id,event_id" });
      }
