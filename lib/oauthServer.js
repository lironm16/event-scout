...
function registerMiniAppRoutes(app, { prefix, verifyInitData, logLabel, bot }) {
    ...
    try {
      ...
      const events = (result.events || []).map((p) => {
        ...
        if (umb) {
          ...
          s.umbrellaImage = umb.image_url || null;
        }
  ...
  app.get(`${prefix}/occurrences`, async (req, res) => {
    ...
    try {
      ...
      const all = await getAllEvents();
      const occ = (all || [])
        // Include the representative too — the list shows ALL occurrences.
        .filter((e) => seriesKey(e) === key && !occurrencePassed(e.date, e.start_time, e.end_time))
