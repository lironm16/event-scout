    ...
    try {
      ...
      try {
        ...
        let cq = supabase
          ...
          .eq("name", ev.name); // series = same-name recurrence (not umbrella)
  ...
  app.get(`${prefix}/occurrences`, async (req, res) => {
    ...
    try {
      ...
      let q = supabase
        ...
        .limit(40);
      // SERIES = the same event recurring → group by exact name. (Umbrella /
      // program grouping is handled separately by the purple link / /umbrella.)
      q = q.eq("name", ev.name);
