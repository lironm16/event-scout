...
function registerMiniAppRoutes(app, { prefix, verifyInitData, logLabel, bot }) {
  ...
  app.get(`${prefix}/event`, async (req, res) => {
    ...
    try {
      ...
      // Count series occurrences so the card shows "🔁 מופעים נוספים" (parity
      // with list view, which gets this from search). Same identity as the
      // /occurrences endpoint: same umbrella, else same name; upcoming only.
      try {
        const today = new Date().toISOString().slice(0, 10);
        let cq = supabase
          .from("events").select("id, date, start_time, end_time")
          .eq("archived", false).gte("date", today).limit(500);
        cq = ev.umbrella_slug ? cq.eq("umbrella_slug", ev.umbrella_slug) : cq.eq("name", ev.name);
        const { data: occRows } = await cq;
        const live = (occRows || []).filter((r) => !occurrencePassed(r.date, r.start_time, r.end_time));
        if (live.length > 0) serialized.totalOccurrences = live.length;
      } catch { /* non-fatal */ }
  ...
  app.get(`${prefix}/occurrences`, async (req, res) => {
    ...
    try {
      ...
      let q = supabase
        .from("events")
        .select("id, name, date, start_time, end_time, tickets_left, location_key, umbrella_slug, external_url, source, locations:location_key(raw_address)")
        .gte("date", today)
        .order("date", { ascending: true })
        .order("start_time", { ascending: true })
        .limit(40);
      // Series identity: same umbrella, else same exact name.
      if (ev.umbrella_slug) q = q.eq("umbrella_slug", ev.umbrella_slug);
      else q = q.eq("name", ev.name);
      const { data: rows, error } = await q;
      if (error) throw new Error(error.message);
      const occ = (rows || [])
        .filter((r) => r.id !== id && !occurrencePassed(r.date, r.start_time, r.end_time))
        .map((r) => ({
          id: r.id,
          dateHe: formatHebrewDate(r.date),
          timeHe: formatTimeRange(r.start_time, r.end_time),
          location: r.locations?.raw_address || null,
          ticketsLeft: r.tickets_left,
          bookingUrl: getBookingUrl(r),
        }));
