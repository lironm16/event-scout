...
function registerMiniAppRoutes(app, { prefix, verifyInitData, logLabel, bot }) {
    ...
    try {
      ...
      const events = (result.events || []).map((p) => {
        ...
        if (occList.length) {
          ...
          // Lightweight sibling list so the catalog autocomplete can suggest each
          // individual occurrence by name (e.g. each "קיץ של בלונים בגינת X")
          // and open THAT specific event — without un-collapsing the parent card.
          if (s.totalOccurrences > 1) {
            s.occurrenceList = occList
              .map((o) => ({ id: o.id, name: o.name || null, location: o.location || null }))
              .filter((o) => o.id);
  ...
  app.get(`${prefix}/occurrences`, async (req, res) => {
    ...
    try {
      ...
      // `windowed` tells the client the list was scoped to the search dates and
      // `totalAll` is the full series size — so it can offer "show all (M)".
      return res.json({ occurrences: occ, totalAll, windowed: occ.length < totalAll });
    } catch (err) {
