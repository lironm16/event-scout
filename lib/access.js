...
const ACCESS_RULES = [
  ...
  },
  {
    ...
    match: /הקהילה\s*הגאה|המרכז\s*הגאה|קהילה\s*גאה|\bpride\b/i,
  },
  {
  ...
  },
  {
    ...
    //     ולמילואימניקים".
    match: /\bמילואים\b|למילואימניק|מילואימניק|\bmiluim\b|\breservists?\b/i,
  },
  {
 ...
 *
 * `text` can be any string. Callers should pre-trim and / or
 * concatenate fields they want considered (e.g. `name + " " + description`
 * for Smarticket, or `category.name + " | " + cluster.name` for the
 * city CMS).
 */
 ...
 *
 * We don't fall back to 'open' here — emitting 'open' would force
 * the upsert to OVERWRITE any manual classification someone made
 * via the SQL console. Returning null lets the caller decide
 * (today: skip the field entirely so the DB row keeps its
 * previous value, defaulting to 'open' only on first insert).
 */
function classifyAccessForEvent({ name, description }) {
  // Title is the strongest signal — venues advertise community
  // membership prominently. Description is the fallback for the
  // rare generic title ("Friday Night Mixer") where the body
  // text is the only place the audience is specified.
  return (
