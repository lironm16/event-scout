...
// weekly newsletter) when an event drops to ≤10 tickets.
//
// The spec rule (May-2026):
//   "If an event hits the 10-ticket threshold, the system must send
...
//
//   1. tickets_left was > 10 → now ≤ 10  (the user-visible "drop")
//   2. tickets_left was NULL/unknown → now ≤ 10  (event arrived
...
const { formatLowStockBadge, buildNavButtons } = require("./eventCard");
...
function buildLowStockKeyboard(event) {
  const rows = [];
...
async function notifyLowStockMatchesFor(events, telegram) {
  ...
  for (const event of events) {
    ...
    for (const tg of candidateIds) {
      ...
      const text = buildLowStockMessage(event, reason, nameByTg.get(tg));
      const reply_markup = buildLowStockKeyboard(event);
