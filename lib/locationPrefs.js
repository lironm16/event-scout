...
/** Within «נסיעה קצרה» (≤10 min). Walkable venues always pass. */
function isWithinShortDrive(result) {
  if (!result?.resolved) return false;
  if (!result.requires_car) return true;
  if (result.drive_minutes != null && result.drive_minutes <= DRIVE_MAX_MINUTES) {
    return true;
  }
  ...
  if (result.km != null && result.km <= SHORT_DRIVE_KM) return true;
  return false;
}
...
function eventPassesLocationModes(result, constraints) {
  ...
  if (modes.includes("walk") && modes.includes("drive")) {
    return !result.requires_car || isWithinShortDrive(result);
  }
  if (modes.includes("walk")) return !result.requires_car;
  if (modes.includes("drive")) return isWithinShortDrive(result);
  return true;
}
