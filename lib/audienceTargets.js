...
function audienceSetFromChipIds(chipIds) {
  // Strict: match exactly the selected audiences. "לכל המשפחה" is now a
  // first-class chip the user can pick explicitly, so we no longer force it
  // in as a baseline — selecting only ותיקים must return only ותיקים.
...
function hydrateTargetAudienceChipIds(profile) {
  ...
  if (Array.isArray(kids) && kids.length > 0) {
    selected.add("ילדים");
    selected.add("לכל המשפחה"); // family events are relevant to anyone with kids
