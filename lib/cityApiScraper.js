...
async function upsertCityEventRow(row, logger) {
  ...
  // The classifier is monotonic: when it detects a community signal
  // (returns 'community-X') that's the new truth and we write it; when
  // it returns 'open' it means "no signal seen", which is a WEAKER
  // statement than "open" stored in the DB by a backfill or manual
  // SQL edit. Preserving the existing value protects us from three
  // race classes:
  //   1. Classifier upgrades: backfill set rows to 'community-seniors'
  //      using umbrella context; the still-deployed older scraper
  //      runs `mapAccess` and gets 'open' because IT doesn't read
  //      umbrella. Without this guard, every cycle erases the
  //      backfill.
  //   2. Manual ops edits via Supabase SQL editor.
  //   3. A future enricher that decides access from description /
  //      external sources — its writes mustn't be erased on the next
  //      scrape cycle.
  // A genuine 'open' → 'community-X' upgrade still happens (the
  // scraper's non-'open' result is authoritative).
  if (
    persistable.access === "open" &&
    existingRow?.access &&
    existingRow.access !== "open"
  ) {
