...
async function scrapeCityApi(opts = {}) {
  ...
  try {
    const { fillMissingLocationsFromLobby } = require("./lobbyLocationFill");
    const { matched, filled } = await fillMissingLocationsFromLobby(lobby, { dryRun, logger });
    logger.log(`[CityApi] cross-source location fill: matched ${matched}, filled ${filled}`);
  } catch (err) {
    logger.warn(`[CityApi] location fill failed (non-fatal): ${err.message}`);
  }
