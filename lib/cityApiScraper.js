...
const { isAdminEntry } = require("./timeContext");
...
async function upsertUmbrellaRow(lobbyEntry, detailJson, source, logger) {
  ...
  // Parent (programme) description — shown on the series card. Falls back to
  // null; never overwrites an existing description with empty.
  let description = null;
  try { description = extractCityDescription(detailJson) || null; } catch { /* ok */ }
...
async function upsertCityEventRow(row, logger) {
  ...
  // Skip admin/test stubs at ingestion (symmetry with the Smarticket path in
  // api/check.js) — don't even write them. Returning true = "handled", not an
  // error (the caller only buckets ok vs error).
  if (isAdminEntry(persistable.name)) {
    logger.info?.(`[CityApi] skipping admin/test entry: ${persistable.name}`);
    return true;
  }
...
async function scrapeCityApi(opts = {}) {
  ...
  // Cross-source enrichment: the smarticket-backed lobby entries we "skip" for
  // ingestion still carry a structured eventLocation that the Smarticket feed
  // lacked. Fill location_key on those duplicate (source="ramat-gan"/"mbe-rg")
  // events from the lobby — runs every city scrape, right after the Smarticket
  // scrape. (Match by title+date; no Gemini.)
  try {
    const { fillMissingFieldsFromLobby } = require("./lobbyLocationFill");
    const { matched, locations, images } = await fillMissingFieldsFromLobby(lobby, { dryRun, logger });
    logger.log(`[CityApi] cross-source fill: matched ${matched} (locations ${locations}, images ${images})`);
  } catch (err) {
    logger.warn(`[CityApi] cross-source fill failed (non-fatal): ${err.message}`);
  }
