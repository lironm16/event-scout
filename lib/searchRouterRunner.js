...
async function runSearchWithFilters(telegramId, agentCtx, filters) {
  ...
  if (matched === 0) {
    ...
    await ctx.tg.reply(msg, searchMenuKeyboard(gender));
    if (result?.can_extend_beyond_window && result?.extension_hint) {
      await ctx.tg.reply(
        "להרחיב את החיפוש?",
  ...
  if (result?.can_extend_beyond_window && result?.extension_hint) {
    ...
    const extra = hint.count_at_least || "עוד";
    const when = hint.label_he || "מאוחר יותר";
    await ctx.tg.reply(
      `יש לפחות ${extra} אירועים ${when} — להרחיב את החיפוש?`,
      Markup.inlineKeyboard([[Markup.button.callback("כן, להרחיב", "rtr:extend")]]),
    );
  }
