...
const VALID_KID_GENDERS = new Set(["female", "male"]);
...
function normalizeKidGender(value) {
  if (value === "female" || value === "male") return value;
  return null;
}

function formatKidGenderHe(gender) {
  if (gender === "female") return "בת";
  if (gender === "male") return "בן";
  return null;
}

function formatKidBornHe(gender) {
  const g = normalizeKidGender(gender);
  if (g === "male") return "נולד";
  if (g === "female") return "נולדה";
  return "נולד";
}
...
function formatKidProfileSuffix(kid, asOf = new Date()) {
  ...
  const parts = [];
  const g = formatKidGenderHe(normalizeKidGender(kid.gender));
  if (g) parts.push(g);
  if (iso) parts.push(`${formatKidBornHe(kid.gender)} ${formatBirthDateHe(iso)}`);
...
function normalizeKidForStorage(kid) {
  ...
  const gender = normalizeKidGender(kid.gender);
  const out = {};
  if (name) out.name = name;
  else if (birth_date) out.name = kidNounHe(gender);
  ...
  if (!out.name && !out.birth_date) {
    ...
    if (Number.isFinite(age) && age >= 0) {
      out.age = age;
      if (!out.name) out.name = kidNounHe(out.gender);
    }
  }
...
/** בן / בת / ילד — after gender is chosen, prefer this over neutral. */
function kidNounHe(gender) {
  const g = formatKidGenderHe(normalizeKidGender(gender));
  return g || "ילד";
}

function kidNounDefiniteHe(gender) {
  const n = kidNounHe(gender);
  if (n === "בן") return "הבן";
  if (n === "בת") return "הבת";
  return "הילד";
}

function kidNameStepBody(gender) {
  return (
    `✏️ *שם ${kidNounDefiniteHe(gender)}*\n\n` +
    "כתבי שם בהודעה הבאה (למשל: תומר):"
  );
}

function birthDatePromptForKid(gender) {
  const g = normalizeKidGender(gender);
  const head = g
    ? `📅 *תאריך לידה — ${formatKidBornHe(g)}*\n\n`
    : "📅 *תאריך לידה מלא*\n\n";
  return (
    head +
    "כתבי יום.חודש.שנה (למשל `15.3.2024` או `2024-03-15`).\n" +
...
function kidsStagesStepBody(gender, markVerb) {
  return `🧩 *שלבי התפתחות — ${kidNounDefiniteHe(gender)}*\n\n${markVerb} את כל המתאימים:`;
}
